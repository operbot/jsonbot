# jsonbot/plugs/common/overflow.py
#
#
...
from jsonbot.lib.errors import URLNotEnabled
...
from jsonbot.lib.periodical import minutely, periodical
from jsonbot.lib.persistconfig import PersistConfig
from jsonbot.lib.persiststate import PlugState
from jsonbot.utils.exception import handle_exception
from jsonbot.utils.generic import splittxt
from jsonbot.utils.lazydict import LazyDict
from jsonbot.utils.timeutils import today
from jsonbot.utils.url import geturl2, re_url_match, striphtml
...
class OverFlowAPI(object):
    ...
    def search(
        ...
        json_data = self.api(
            "/search?intitle=%s&tagged=%s" % (title, ";".join(tags)), size
        )
...
def scan(skip=False):
    ...
    for b in todo:
        uid = str(b.user_id)
        if not uid in state.data.ids:
            logging.warn("we don't follow id %s" % uid)
            continue
        for channel in state.data.ids[uid]:
            ...
            if bot:
                if b.post_id:
                    url = (
                        "http://stackoverflow.com/questions/%s" % b.post_id
                    ) or "no url found"
                bot.say(
                    chan,
                    "*%s* %s - *%s* - %s - %s - %s (%s)"
                    % (
                        state.data.names[uid].upper(),
                        b.action,
                        b.description,
                        url,
                        time.ctime(b.creation_date),
                        b.detail or "no detail",
                        b.post_type,
                    ),
                )
                if b.action == "answered":
                    ...
                    if aa:
                        ...
                        if c:
                            ...
                            bot.say(chan, "can't find answers")
                        if urls:
                            bot.say(chan, "urls: %s" % " -=- ".join(urls))
            else:
...
def handle_overflowstop(bot, event):
    ...
    try:
        ...
        event.reply("we are not monitoring %s in %s" % (id, event.channel))
...
def handle_overflownames(bot, event):
    event.reply("names list: ", state.data.names)


cmnds.add(
    "overflow-names",
    handle_overflownames,
    [
        "OPER",
    ],
)
