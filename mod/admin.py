...
from jsonbot.lib.aliases import setalias
from jsonbot.lib.boot import (boot, clear_tables, getcallbacktable, getcmndtable,
...
from jsonbot.utils.generic import getwho, stringsed
...
def handle_adminboot(bot, ievent):
    ...
    if "saveperms" in ievent.rest:
        boot(force=True, saveperms=True, clear=True)
    else:
        boot(force=True, saveperms=False, clear=True)
    ievent.done()
...
examples.add(
    "admin-bootthreaded",
    "initialize the bot in a seperate thread",
    "admin-bootthreaded",
)
...
def handle_admincommands(bot, ievent):
    ...
    cmnds = getcmndtable()
    if not ievent.rest:
        ievent.reply("commands: ", cmnds)
    else:
        try:
            ievent.reply(
                "%s command is found in %s " % (ievent.rest, cmnds[ievent.rest])
            )
        except KeyError:
...
def handle_admincallbacks(bot, ievent):
    ...
    if not ievent.rest:
        ievent.reply("callbacks: ", cbs)
    else:
        try:
            ievent.reply("%s callbacks: " % ievent.rest, cbs[ievent.rest])
        except KeyError:
            ievent.reply("no such callbacks available")
...
def handle_userhostscache(bot, ievent):
    ...
    ievent.reply(
        "userhostcache of %s: " % ievent.channel,
        list(bot.userhosts.keys()) + list(bot.userhosts.values()),
    )
...
def handle_adminstop(bot, ievent):
    ...
    if bot.isgae:
        ...
        return
    if bot.ownercheck(ievent.userhost):
...
def handle_adminupgrade(bot, event):
    ...
    if not bot.isgae:
        event.reply("this command only works in GAE")
        return
    else:
        import google
    from jsonbot.lib.persist import JSONindb
    ...
    for d in JSONindb.all():
        try:
            dd = d.filename
            if not "gozerdata" in dd:
                continue
            if "run" in dd:
                continue
            ddd = stringsed(dd, "s/%s/%s/" % ("gozerdata", "data"))
            ddd = stringsed(ddd, "s/waveplugs/jsonbot.plugs.wave/")
            ddd = stringsed(ddd, "s/gozerlib\.plugs/jsonbot.plugs.core/")
            ddd = stringsed(ddd, "s/commonplugs/jsonbot.plugs.common/")
            ddd = stringsed(ddd, "s/socketplugs/jsonbot.plugs.socket/")
            ddd = stringsed(ddd, "s/gaeplugs/jsonbot.plugs.gae/")
            if d.get_by_key_name(ddd):
                continue
            d.filename = ddd
            kwds = {}
            for prop in props:
                kwds[prop] = getattr(d, prop)
            d.get_or_insert(ddd, **kwds)
            bot.say(event.channel, "UPGRADED %s" % ddd)
            teller += 1
        except Exception as ex:
            bot.say(event.channel, str(ex))
...
def handle_adminexceptions(bot, event):
    ...
    from jsonbot.utils.exception import exceptionevents, exceptionlist
...
def handle_adminmc(bot, event):
    ...
    if event.rest == "stats":
        try:
            from jsonbot.memcached import mc

            test = mc.get_stats()
            if not test:
                event.reply("no memcached found")
            else:
                event.reply("memcached stats: ", test[0][1])
        except Exception as ex:
        ...
        try:
            ...
            if mc:
                ...
                event.reply("no memcached running")
        except Exception as ex:
            event.reply("memcached error: %s" % str(ex))
    else:
        event.reply("choose one of stats, flushall")
...
def handle_adminfloodcontrol(bot, event):
    ...
    try:
        ...
        event.missing("<userhost> <threshold> [period] [wait]")
        return
    userhost = getwho(bot, who)
    if userhost:
        user = bot.users.getuser(userhost)
    else:
        user = bot.users.byname(who)
    if not user:
        event.reply("i don't know a user %s" % who)
        return
    if "OPER" in user.data.perms:
        event.reply("no flood control for OPER")
        return
    try:
        period = event.args[2]
    except IndexError:
    ...
    user.save()
    from jsonbot.lib.floodcontrol import floodcontrol

    for u in user.data.userhosts:
        floodcontrol.reset(u)
    event.reply("floodrate for %s set to %s" % (user.data.name, threshold))


cmnds.add(
    "admin-floodcontrol",
    handle_adminfloodcontrol,
    [
        "OPER",
    ],
)
examples.add(
    "admin-floodcontrol",
    "set the floodcontrol parameters of a user",
