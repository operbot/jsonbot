...
def handle_yacy(bot, event):
    ...
    for h in prio1 + hosts:
        ...
        try:
            r = json.loads(
                getresults(queryurl % (h, urllib.parse.quote_plus(event.rest)))
            )
            if r:
                got = h
                break
    ...
    for channel in r["channels"]:
        for item in channel["items"]:
            result.append("%s - %s (%s)" % (item["title"], item["link"], item["size"]))
    if result:
        event.reply("results from %s: " % got, result, dot=" || ")
    else:
