# jsonbot/plugs/socket/fisheye.py
#
#
...
def getRpcClient(project):
    if project["name"] not in rpc_clients:
        ...
        logging.info(
            "Created new RPC client for %s with auth token: %s"
            % (project["name"], auth)
        )
...
def containsHash(bot, ievent):
    ...
    if ievent.channel in cfg.data and len(cfg.data[ievent.channel]):
        if gitHashRule.match(ievent.txt):
            return 1
    return 0
...
def handle_add_fisheye_project(bot, ievent):
    ...
    if len(ievent.args) != 4:
        ievent.reply(
            "syntax: add_fisheye_project [project name] [url] [username] [password]"
        )
        return
...
examples.add(
    ...
    "enable fisheye commit lookups in the channel",
    "fisheye_commit_lookup_enable",
)
...
cmnds.add(
    "fisheye_commit_lookup_disable", handle_fisheye_commit_lookup_disable, ["OPER"]
)
examples.add(
    "fisheye_commit_lookup_disable",
    "disable fisheye commit lookups in the channel",
    "fisheye_commit_lookup_disable",
)
