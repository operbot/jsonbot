...
from jsonbot.lib.persist import Persist, PersistCollection, PlugPersist
from jsonbot.lib.persistconfig import PersistConfig
from jsonbot.lib.plugins import plugs as plugins
from jsonbot.lib.threads import start_new_thread
from jsonbot.utils.exception import handle_exception
from jsonbot.utils.generic import jsonstring
from jsonbot.utils.limlist import Limlist
from jsonbot.utils.url import geturl, striphtml
...
def iscommand(bot, ievent):
    ...
    if cc and ievent.txt[0] == cc:
        txt = ievent.txt[1:]
    if ievent.txt.startswith(bot.nick + ":") or ievent.txt.startswith(bot.nick + ","):
...
def cb_markovtalk(bot, ievent):
    ...
    # markovtalk_learn
    if enabled(bot.cfg.name, ievent.channel):
...
examples.add(
    ...
    "1) markov-learnadd #dunkbots 2) markov-learnadd http://jsonbot.org",
)
...
def handle_markovlearndel(bot, ievent):
    ...
    try:
        ...
        return
    if item not in markovlearn.data.l:
...
examples.add(
    "markov-learndel",
    "remove item from learnlist",
    "1) markov-learndel #dunkbots 2) markov-learndel http://jsonbot.org",
)


def handle_markov(bot, ievent):
    """this is the command to make the bot reply a markov response"""
    if not enabled(bot.cfg.name, ievent.channel):
...
def handle_markovonjoinadd(bot, ievent):
    ...
    if (bot.cfg.name, channel) in cfg.get("onjoin"):
...
def handle_markovonjoinremove(bot, ievent):
    ...
    try:
        ...
        channel = ievent.channel
    try:
        cfg.get("onjoin").remove((bot.cfg.name, channel))
    except ValueError:
...
examples.add(
    ...
    "1) markov-enable 2) markov-enable #dunkbots",
)
...
examples.add(
    ...
    "1) markov-disable 2) markov-disable #dunkbots",
)
