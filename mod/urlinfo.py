# jsonbot/plugs/common/urlinfo.py
# -*- coding: utf-8 -*-
#
#
        ...
        for i in urls:
            ...
            try:
                server = xmlrpc.client.ServerProxy(
                    "http://whatisthisfile.appspot.com/xmlrpc"
                )
                logging.info("urlinfo - XMLRPC query: %s" % i)
                urlinfo = server.app.query(i)
                if "html" in urlinfo:
                    if "title" in urlinfo["html"]:
                        o += 'Title: "%s" ' % urlinfo["html"]["title"].strip()
                elif "image" in urlinfo:
                    o += "Image: %dx%d " % (
                        urlinfo["image"]["width"],
                        urlinfo["image"]["height"],
                    )
                if not o:
                    continue
                if len(o):
                ...
                if "tinyurl" in i:
                    out = out.strip()
                    out += " - %s" % i
                elif not "http://" in out:
                    out = out.strip()
                    out += " - %s" % get_tinyurl(i)[0]
            except Exception:
                pass
    return out.strip()
...
def handle_urlinfo_list(bot, ievent):
    ...
    if not names:
        ...
        ievent.reply("urlinfo enabled on channels: ", names)


cmnds.add("urlinfo-list", handle_urlinfo_list, "OPER")
examples.add(
    "urlinfo-list", "show in which channels urlinfo is enabled", "urlinfo-list"
)
