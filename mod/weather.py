# jsonbot/plugs/common/weather.py
#
#
...
from jsonbot.utils.url import geturl
    ...
    if weathertxt:
        ...
        if ievent.usercmnd == "weather":
            ...
            if info:
                ...
                time = (
                    info.getElementsByTagName("current_date_time")[0]
                    .attributes["data"]
                    .value
                )
                weather = gweather.getElementsByTagName("current_conditions")[0]
                condition = (
                    weather.getElementsByTagName("condition")[0]
                    .attributes["data"]
                    .value
                )
                temp_f = (
                ...
                temp_c = (
                    weather.getElementsByTagName("temp_c")[0].attributes["data"].value
                )
                humidity = (
                    weather.getElementsByTagName("humidity")[0].attributes["data"].value
                )
                try:
                    wind = (
                        weather.getElementsByTagName("wind_condition")[0]
                        .attributes["data"]
                        .value
                    )
                except IndexError:
                ...
                resultstr = (
                    "As of %s, %s (%s) has a temperature of %sC/%sF with %s. %s (%s km/h).%s"
                    % (
                        time,
                        city,
                        zip,
            ...
            for forecast in forecasts:
                condition = (
                    forecast.getElementsByTagName("condition")[0]
                    .attributes["data"]
                    .value
                )
                low_f = forecast.getElementsByTagName("low")[0].attributes["data"].value
                high_f = (
                    forecast.getElementsByTagName("high")[0].attributes["data"].value
                )
                day = (
                    forecast.getElementsByTagName("day_of_week")[0]
                    .attributes["data"]
                    .value
                )
...
examples.add(
    "weather",
    "get weather for <LOCATION> or <nick>",
    "1) weather London, England 2) weather dunker",
)
