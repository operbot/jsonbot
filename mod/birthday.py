...
from jsonbot.utils.timeutils import bdmonths, elapsedstring, getdaymonth, strtotime
...
def handle_bdset(bot, ievent):
    ...
    global db
    if not db:
        ievent.reply("plugin isnt initialised yet")
        return
    try:
        what = ievent.args[0]
    ...
    try:
        db.execute(
            """ INSERT INTO birthday (name, birthday) VALUES(%s, %s) """, (name, what)
        )
    except:
        try:
            db.execute(
                """ UPDATE birthday SET birthday = %s WHERE name = %s """, (what, name)
            )
...
def handle_bddel(bot, ievent):
    ...
    result = 0
    try:
        result = db.execute(""" DELETE FROM birthday WHERE name = %s """, name)
    except Exception as ex:
        handle_exception()
        return
    if result:
...
def handle_bd(bot, ievent):
    ...
    try:
        ...
        try:
            userhost = who = ievent.args[0].lower()
        except IndexError:
            userhost = who = ievent.userhost
    if not who:
        who = userhost = getwho(bot, who)
    if not who:
...
examples.add(
    "bd",
    "bd [<nr|user>] .. show birthdays for this month or show birthday of <nick>",
    "1) bd 2) bd dunker",
)
    ...
    for i in bds:
        ...
        if month == nowmonth:
            ...
            if day == nowday and month == nowmonth:
                ievent.reply("it's %s's birthday today!" % i[0])
