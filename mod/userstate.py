# jsonbot/plugs/core/userstate.py
#
#
...
from jsonbot.lib.errors import NoSuchUser
...
def handle_get(bot, ievent):
    ...
    for i, j in userstate.data.items():
        if target == i or not target:
            result.append("%s=%s" % (i, j))
    if result:
        ievent.reply("state: ", result)
    else:
        ievent.reply("no userstate of %s known" % ievent.userhost)
...
def handle_unset(bot, ievent):
    ...
    try:
        ...
        return
    try:
        del ievent.user.state.data[item]
    except KeyError:
