# jsonbot/plugs/common/ipcalc.py
#
...
class IP(object):
    ...
    def _dqtoi(self, dq):
        ...
        if ":" in dq:
            ...
            if len(hx) > 8:
                ...
                if not "" in hx:
                    raise ValueError(
                        "%r: IPv6 address invalid: compressed format malformed" % dq
                    )
                elif (
                ...
                raise ValueError(
                    "%r: IPv6 address invalid: compressed format detected in full notation"
                    % dq
                )
            ip = ""
            ...
            for h in hx:
                ...
                if 0 > int(h, 16) > 0xFFFF:
                    raise ValueError(
                        "%r: IPv6 address invalid: hextets should be between 0x0000 and 0xffff"
                        % dq
                    )
                ip += h
            self.v = 6
        ...
        if "." in dq:
            ...
            for x in q:
                if 0 > int(x) > 255:
                    raise ValueError(
                        "%r: IPv4 address invalid: bytes should be between 0 and 255"
                        % dq
                    )
            self.v = 4
    ...
    def _itodq(self, n):
        ...
        if self.v == 4:
            return ".".join(
                map(
                    str, [(n >> 24) & 0xFF, (n >> 16) & 0xFF, (n >> 8) & 0xFF, n & 0xFF]
                )
            )
        else:
            n = "%032x" % n
    ...
    def to_ipv4(self):
        ...
        if self.v == 4:
            ...
            if self.bin().startswith("0" * 96):
                ...
                return IP(
                    (int(self) - 0x20020000000000000000000000000000) >> 80, version=4
                )
            else:
                return (
                    ValueError,
                    "%r: IPv6 address is not IPv4 compatible, nor a 6-to-4 IP"
                    % self.dq,
                )
    ...
    def to_ipv6(self, type="6-to-4"):
        ...
        if self.v == 4:
            if type == "6-to-4":
                return IP(
                    0x20020000000000000000000000000000 | int(self) << 80, version=6
                )
            elif type == "compat":
...
class Network(IP):
    ...
    def netmask(self):
        ...
        if self.version() == 4:
            return IP(
                (0xFFFFFFFF >> (32 - self.mask)) << (32 - self.mask),
                version=self.version(),
            )
        else:
            return IP(
                (0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF >> (128 - self.mask))
                << (128 - self.mask),
                version=self.version(),
            )
    ...
    def broadcast(self):
        ...
        if self.version() == 4:
            return IP(
                int(self.network()) | (0xFFFFFFFF - int(self.netmask())),
                version=self.version(),
            )
        else:
            return IP(
                int(self.network())
                | (0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF - int(self.netmask())),
                version=self.version(),
            )
    ...
    def in_network(self, other):
        ...
        return (
            int(other) >= int(self)
            and int(other) < int(self) + self.size() - other.size() + 1
        )
...
def handle_ipcalc(bot, ievent):
    ...
    try:
        ...
        return
    ievent.reply(
        "version: %d, address: %s, network size: %d, network address: %s, netmask: %s, first host in network: %s, last host in network: %s, network info: %s"
        % (
            net.version(),
            str(net),
            net.mask,
