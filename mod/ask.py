...
def handle_ask(bot, event):
    ...
    if xmppbot:
        for expert in expertslist:
            xmppbot.say(
                expert,
                "%s (%s) asks you: %s" % (event.userhost, bot.cfg.name, question),
            )
    else:
    ...
    for expert in expertslist:
        ...
        questions.data[expert].append(
            [bot.cfg.name, bot.type, event.userhost, event.channel]
        )
    questions.save()
...
examples.add(
    "ask",
    "ask [group|JID] question .. ask a groups of users a question or use a specific JID",
...
def handle_askjoin(bot, event):
    ...
    try:
        ...
        return
    if subject not in experts.data:
    ...
    if expert not in subjects.data:
...
examples.add(
    "ask-subjects",
    "list all the subjects an expert handles",
    "ask-subjects bthate@gmail.com",
)
