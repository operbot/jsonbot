...
def handle_cfgset(bot, event):
    if len(event.args) != 3:
        event.missing("<configname> <variable> <value>")
        return
    name, var, value = event.args
    if not var in allowset:
        event.reply("setting %s is not allowed" % var)
        return
    if name == "main":
        if not getusers().allowed(event.userhost, "OPER"):
            event.reply("you need to have OPER permissions to edit the mainconfig.")
            return
        mcfg = getmainconfig()
        try:
            mcfg[var] = int(value)
        ...
        event.reply('we current only support editing the "main" config.')
        return
