# jsonbot/plugs/common/watcher.py
#
#
...
from jsonbot.utils.format import formatevent
from jsonbot.utils.generic import stripcolor
from jsonbot.utils.locking import locked
...
def writeout(botname, type, channel, txt, eventjson):
    ...
    watchbot = None
    if botname in bots:
        watchbot = bots[botname]
    if not watchbot:
        watchbot = getfleet().byname(botname)
    if not watchbot:
        watchbot = getfleet().makebot(type, botname)
    if watchbot:
        if not botname in bots:
            bots[botname] = watchbot
...
def prewatchcallback(bot, event):
    ...
    if not event.txt:
        return
    return (
        watched.check(str(event.channel))
        and event.how != "background"
        and event.forwarded
    )
    ...
    for item in subscribers:
        try:
            try:
                ...
                continue
            if not event.allowatch:
                pass
            elif channel not in event.allowwatch:
                logging.warn(
                    "watcher - allowwatch denied %s - %s" % (channel, event.allowwatch)
                )
                continue
            m = formatevent(bot, event, subscribers, True)
            if event.cbtype in ["OUTPUT", "JOIN", "PART", "QUIT", "NICK"]:
                txt = "[!] %s" % m.txt
            else:
                txt = "[%s] %s" % (m.nick or event.nick or event.auth, m.txt)
            if txt.count("] [") > 2:
...
def handle_watcherstop(bot, event):
    ...
    if not event.rest:
        ...
        target = event.rest.strip()
    try:
        watched.unsubscribe(bot.cfg.name, bot.type, target, event.channel)
    except Exception as ex:
        event.reply("error removing watcher: %s" % str(ex))
        return
...
examples.add(
    "watcher-subscribers", "show channels that are watching us. ", "watcher-list"
)
