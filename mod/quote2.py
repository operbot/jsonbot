...
from jsonbot.db.direct import Db
...
class QuotesDb(object):
    ...
    def add(self, nick, userhost, quote):
        ...
        if not db:
            ...
            return 0
        result = db.execute(
            """ INSERT INTO quotes(quote, userhost, createtime, nick) VALUES (%s, %s, %s, %s) """,
            (quote, userhost, time.time(), nick),
        )
        return result
    ...
    def idquote(self, quotenr):
        ...
        if quotenr == 0:
            return
        result = db.execute(
            """ SELECT indx, quote FROM quotes WHERE indx = %s """, quotenr
        )
        if result:
    ...
    def whoquote(self, quotenr):
        ...
        if not db:
            ...
            return
        result = db.execute(
            """ SELECT nick, createtime FROM quotes WHERE indx = %s """, (quotenr,)
        )
        if result:
            return result[0]
    ...
    def last(self, nr=1):
        ...
        if not db:
            ...
            return []
        result = db.execute(
            """ SELECT indx, quote FROM quotes ORDER BY indx DESC LIMIT %s """, (nr,)
        )
        res = []
        if result:
            for i in result:
    ...
    def search(self, what):
        ...
        if not db:
            ...
            return []
        result = db.execute(
            """ SELECT indx, quote FROM quotes WHERE quote LIKE %s """, "%%%s%%" % what
        )
        res = []
        if result:
            for i in result:
    ...
    def searchlast(self, what, nr):
        ...
        if not db:
            ...
            return []
        result = db.execute(
            """ SELECT indx, quote FROM quotes WHERE quote LIKE %s ORDER BY indx DESC LIMIT %s """,
            ("%%%s%%" % what, nr),
        )
        res = []
        if result:
...
def handle_quotelast(bot, ievent):
    ...
    if quotelist != None:
        for quote in quotelist:
            ...
            if karma:
                qkarma = karma.karma.get("quote %s" % quote.id)
            else:
                qkarma = None
            if qkarma:
...
def handle_quotegood(bot, ievent):
    ...
    karma = plugs.get("jsonbot.plugs.db.karma2")
    if not karma:
        ievent.reply("karma plugin is not loaded")
        return
