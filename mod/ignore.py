...
def handle_ignore(bot, event):
    ...
    if not event.rest:
        event.missing("<nick>|<userhost>|<JID>")
        return
    nick = event.rest
    ...
    if perms and "OPER" in perms:
        event.reply("can't ignore OPER")
        return
    if not userhost in bot.ignore:
        bot.ignore.append(userhost)
...
def handle_unignore(bot, event):
    ...
    if not userhost:
        userhost = event.rest
    if userhost in bot.ignore:
        bot.ignore.remove(userhost)
        event.reply("%s removed from ignore list" % userhost)
    else:
        event.reply("%s is not in ignore list" % event.rest)
