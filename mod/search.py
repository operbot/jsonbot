# jsonbot/plugs/common/search.py
#
#
...
from jsonbot.lib.persist import Persist, PersistCollection
...
def stats_response(stats, target=[], skip=[]):
    ...
    for res in todo:
        ...
        if not url.endswith(".html"):
            continue
        if "genindex" in str(url):
            continue
        splitted = url.split("/")
        if target and target[-1] in splitted[-1]:
            first.append("%s (%s)" % (url, count))
        else:
            r.append("%s (%s)" % (url, count))
    first.extend(r)
    return first
...
def handle_search(bot, event):
    ...
    target = event.args
    if not target:
        event.missing("<search words seperated by space>")
        return
    ...
    if files:
        for f in files:
            try:
                res.append(Persist(f).data.url)
            except AttributeError as ex:
                continue
    objs = coll.search("txt", event.rest)
    if not objs:
        objs = list(coll.objects().values())
    stats = makestats(objs, target, res)
    urls = stats_response(stats, target)
    res.extend(urls)
    if res:
        if len(res) < 4 or all:
            event.reply("found %s urls: " % len(res), res, dot=" -or- ")
        else:
            event.reply(
                "found %s urls, use --all for more: " % len(res), res[:3], dot=" -or- "
            )
    else:
