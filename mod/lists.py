...
def getlist(username, listname):
    ...
    if not db:
        ...
        return []
    result = db.execute(
        """ SELECT * FROM list WHERE username = %s AND listname = %s """,
        (username, listname),
    )
    return result
...
def addtolist(username, listname, item):
    ...
    if not db:
        ...
        return 0
    result = db.execute(
        """ INSERT INTO list(username, listname, item) VALUES(%s, %s, %s) """,
        (username, listname, item),
    )
    return result
...
def delfromlist(username, indx):
    ...
    if not db:
        ...
        return 0
    result = db.execute(
        """ DELETE FROM list WHERE username = %s AND indx = %s """, (username, indx)
    )
    return result
...
examples.add(
    "lists",
    "show content of list or add item to list",
...
examples.add(
    "lists-del",
    "remove items with indexnr from list",
