...
def makedirs(ddir=None):
    ...
    try:
        doit(ddir, "jsb.plugs.myplugs")
    except:
        pass
    try:
        doit(ddir, "jsb.data.examples")
    except:
        pass
    try:
        doit(ddir, "jsb.data.static", "static")
    except:
        pass
    try:
        doit(ddir, "jsb.data.templates", "templates")
    except:
        pass
    try:
    ...
    if not os.path.isfile(ddir + os.sep + "config" + os.sep + "mainconfig"):
        ...
        if not source:
            raise Exception("can't find jsb.data.examples package")
        try:
            shutil.copy(
            ...
            logging.error(
                "datadir - failed to copy jsb.data.config.mainconfig: %s" % str(ex)
            )
    if not os.path.isfile(ddir + os.sep + "config" + os.sep + "credentials.py"):
        ...
        try:
            ...
            logging.error("datadir - failed to copy jsb.data.config: %s" % str(ex))
    try:
    ...
    if not initsource:
        raise Exception("can't find jsb.plugs.myplugs package")
