...
from jsonbot.lib.threadloop import RunnerLoop
from jsonbot.lib.threads import getname, start_bot_command, start_new_thread
from jsonbot.utils.exception import handle_exception
from jsonbot.utils.generic import waitevents
from jsonbot.utils.locking import lockdec, locked
from jsonbot.utils.lockmanager import lockmanager, rlockmanager
from jsonbot.utils.trace import callstack, whichmodule
...
class Runner(RunnerLoop):
    ...
    def done(self, event):
        try:
            ...
            if event.cbtype not in ["TICK", "PING", "NOTICE", "TICK60"]:
...
class BotEventRunner(Runner):
    def handle(self, speed, args):
        ...
        try:
            ...
            if not ievent.nolog:
                logging.debug("event handler is %s" % str(func))
            ...
            try:
                ...
                logging.warn("urls fetching is disabled (%s)" % ievent.usercmnd)
                return
            self.finished = time.time()
            self.elapsed = self.finished - self.starttime
            if self.elapsed > 5:
...
class Runners(object):
    ...
    def runnersizes(self):
        ...
        for runner in self.runners:
            result.append("%s - %s" % (runner.queue.qsize(), runner.name))
        return result
...
def size():
    return "cmnd: %s - callbacks: %s - wait: %s - long: %s - api: %s" % (
        cmndrunner.size(),
        callbackrunner.size(),
        waitrunner.size(),
        longrunner.size(),
