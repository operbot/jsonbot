...
from jsonbot.utils.xmpp import stripped
...
class Commands(LazyDict):
    ...
    def add(
        ...
        **kwargs
        ...
        try:
            ...
            prev = None
        target = Command(
            modname, cmnd, func, perms, threaded, wait, orig, how, speed=speed
        )
        if regex:
        ...
        try:
            ...
            if p in self.pre:
                if not self.pre[p]:
                    self.pre[p] = []
                if prev in self.pre[p]:
                    self.pre[p].remove(prev)
                if target not in self.pre[p]:
                    self.pre[p].append(target)
            else:
                self.pre[p] = [
                    target,
                ]
    ...
    def woulddispatch(self, bot, event):
        ...
        try:
            ...
            if event.execstr and not cmnd:
                cmnd = event.execstr.split()[0]
            if not cmnd:
                cmnd = event.txt.split()[0]
        except Exception as ex:
            logging.debug("can't determine command")
            return None
        try:
    ...
    def dispatch(self, bot, event, direct=False):
        ...
        """
        if event.nodispatch:
            logging.info("nodispatch is set on event")
            return
        if event.groupchat and bot.cfg.fulljids:
            id = event.auth
        elif event.groupchat:
        ...
        if not event.user:
            event.bind(bot)
        if not event.user:
            raise NoSuchUser(event.userhost)
        self.reloadcheck(bot, event)
        c = self.woulddispatch(bot, event)
        if not c:
            c = self.checkre(bot, event)
        if not c:
            raise NoSuchCommand(event.usercmnd)
        if c.modname in bot.plugs.loading and bot.plugs.loading[c.modname]:
            event.reply("%s is loading" % c.modname)
            return
        if bot.cmndperms and bot.cmndperms[c.cmnd]:
            perms = bot.cmndperms[c.cmnd]
        else:
            perms = c.perms
        if bot.allowall:
            return self.doit(bot, event, c, direct)
        elif not bot.users or bot.users.allowed(id, perms, bot=bot):
            return self.doit(bot, event, c, direct)
        elif bot.users.allowed(id, perms, bot=bot):
            return self.doit(bot, event, c, direct)
        return event
    ...
    def doit(self, bot, event, target, direct=False):
        ...
        if target.modname in event.chan.data.denyplug:
            logging.warn(
                "%s is denied in channel %s - %s"
                % (target.plugname, event.channel, event.userhost)
            )
            return
        ...
        try:
            if bot.isgae:
                if (
                    not event.notask
                    and (target.threaded or event.threaded)
                    and not event.nothreads
                ):
                    logging.warn("LAUNCHING AS TASK")
                    from jsonbot.drivers.gae.tasks import start_botevent

                    if target.threaded == "backend":
                        start_botevent(bot, event, "backend")
                    else:
                        start_botevent(bot, event, target.speed or event.speed)
                    event.reply("task started for %s" % event.auth)
                else:
                    target.func(bot, event)
            else:
                if direct or event.direct:
                    target.func(bot, event)
                elif target.threaded and not event.nothreads:
                    logging.warning(
                        "launching thread for %s (%s)" % (event.usercmnd, bot.cfg.name)
                    )
    ...
    def reloadcheck(self, bot, event, target=None):
        ...
        from jsonbot.lib.aliases import getaliases
        ...
        if not plugin:
            logging.debug("can't find plugin to reload for %s" % target)
            return
        if plugin in bot.plugs:
            logging.info(" %s already loaded" % plugin)
            return plugloaded
        elif plugin in plugblacklist.data:
            return plugloaded
        elif bot.cfg.loadlist and plugin not in bot.cfg.loadlist:
            logging.warn("plugin %s is blacklisted" % plugin)
            return plugloaded
