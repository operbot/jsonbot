...
from jsonbot.utils.exception import exceptionmsg, handle_exception
from jsonbot.utils.generic import stripped
...
class Users(Persist):
    ...
    def all(self):
        """get all users."""
        result = []
        for name in list(self.data["names"].values()):
            result.append(JsonUser(name).lower())
        return result
    ...
    def byname(self, name):
        ...
        try:
            ...
            # name = stripname(name)
            user = JsonUser(name)
            if user.data.userhosts and not user.data.deleted:
                return user
        except KeyError:
    ...
    def allowed(self, userhost, perms, log=True, bot=None):
        ...
        if not res and log:
            logging.warn("%s perm %s denied (%s)" % (userhost, str(perms), str(uperms)))
        return res
    ...
    def getname(self, userhost):
        ...
        user = self.getuser(userhost)
        if user and not user.data.deleted:
            return user.data.name.lower()
    ...
    def add(self, name, userhosts, perms):
        ...
        newuser.save()
        self.save()
        logging.warn("%s added to user database - %s" % (name, ", ".join(perms)))
        return newuser

    def addguest(self, userhost, username=None):
        if username and self.byname(username):
            username = userhost
    ...
    def adduserhost(self, name, userhost):
        ...
        user = self.byname(name)
        if not user:
            user = self.users[name] = JsonUser(name=name)
        user.data.userhosts.append(userhost)
        user.save()
        self.data.names[userhost] = name
        self.save()
            ...
            if user:
                for userhost in user.data.userhosts:
                    try:
                        ...
                        logging.warn("can't delete %s from names cache" % userhost)
            self.save()
            return True
        except NoSuchUser:
            pass
    ...
    def make_owner(self, userhosts):
        ...
        for userhost in owner:
            ...
            if not username or username != "owner":
                if not self.merge("owner", str(userhost)):
                    self.add(
                        "owner",
                        [
                            str(userhost),
