...
from jsonbot.utils.lazydict import LazyDict  # simplejson imports
...
class Fleet(Persist):
    ...
    def loadall(self, names=[]):
        ...
        for name in target:
            ...
            try:
                ...
                logging.info("%s is not enabled" % name)
                continue
            except KeyError:
                logging.error("no type know for %s bot" % name)
            except Exception as ex:
                handle_exception()
            if bot and bot not in bots:
                bots.append(bot)
        return bots
    ...
    def makebot(self, type, name, config={}, domain="", showerror=False):
        ...
        if not name:
            logging.error(" name is not correct: %s" % name)
            return
        if not type:
            type = self.data.types.get(name)
        if not type:
            logging.error("no type found for %s bot" % name)
            return
        if config:
            logging.debug(
                "making %s (%s) bot - %s - from: %s"
                % (type, name, config.tojson(), whichmodule())
            )
        bot = None
        if not config:
            cfg = Config("fleet" + os.sep + stripname(name) + os.sep + "config")
    ...
    def addbot(self, bot):
        ...
        if not bot in self.bots:
            ...
            logging.error("%s bot is already in fleet" % bot.cfg.name)
            return False
    ...
    def cmndall(self, event, cmnd):
        ...
        for bot in self.bots:
            self.cmnd(event, bot.cfg.name, cmnd)
    ...
    def boot(self, botnames=[], exclude=[]):
        if not botnames:
            botnames = self.getenabled(exclude)
        ...
        for bot in bots:
            ...
            if bot.type not in exclude:
                todo.append(bot)
                done.append(bot.cfg.name)
        if todo:
            start_new_thread(self.startall, (todo,))
        if done:
            logging.warn("fleet bots are %s" % ", ".join(done))
        else:
            logging.warn(
                "no fleet bots are enabled, see %s" % getdatadir()
                + os.sep
                + "config"
                + os.sep
                + "fleet"
    ...
    def startall(self, bots, usethreads=True):
        ...
        for bot in target:
            ...
            if usethreads or bot.type in ["sleek", "tornado"]:
                threads.append(start_new_thread(bot.boot, ()))
                continue
            try:
                bot.boot()
    ...
    def resumebot(self, botcfg, chan=None):
        ...
        if oldbot and botcfg["type"] in ["sxmpp", "convore"]:
            oldbot.exit()
        ...
        bot.start(False)
        self.addbot(bot)
        if chan and chan in bot.state["joinedchannels"]:
            bot.say(chan, "done!")
