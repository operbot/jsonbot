...
from jsonbot.utils.trace import calledfrom, whichplugin
...
class PersistConfig(Config):
    ...
    def __init__(self):
        ...
        from jsonbot.lib.commands import Command, cmnds
    ...
    def cmnd_cfg_edit(self, bot, ievent, args, key, optionvalue):
        ...
        if type(optionvalue) == list:
            ...
            if type(value) == type(optionvalue):
                ...
                ievent.reply(
                    "value %s (%s) is not of the same type as %s (%s)"
                    % (value, type(value), optionvalue, type(optionvalue))
                )
