...
from jsonbot.utils.trace import calledfrom, callstack, whichplugin
...
class Callbacks(object):
    ...
    def add(
        self, what, func, prereq=None, kwargs=None, threaded=False, nr=False, speed=5
    ):
        """add a callback."""
        what = what.upper()
        ...
        if nr != False:
            self.cbs.insert(
                nr, what, Callback(modname, func, prereq, kwargs, threaded, speed)
            )
        else:
    ...
    def callback(self, cb, bot, event):
        ...
        try:
            ...
            if event.chan and cb.plugname in event.chan.data.denyplug:
                logging.debug(
                    "%s denied in %s - %s" % (cb.modname, event.channel, event.auth)
                )
                return
            if cb.prereq:
                if not event.nolog:
            ...
            if not event.nolog:
                logging.debug(
                    "%s - %s - trail - %s"
                    % (bot.cfg.name, getname(cb.func), callstack(sys._getframe())[::-1])
                )
            ...
            if cb.threaded and not bot.isgae:
                ...
                if event.cbtype == "API":
                    ...
                    apirunner.put(
                        event.speed or cb.speed, cb.modname, cb.func, bot, event
                    )
                elif bot.isgae or event.direct:
                    cb.func(bot, event)
    ...
    def reloadcheck(self, bot, event, target=None):
        ...
        if not event.nolog:
            logging.debug("%s - checking for %s events" % (bot.cfg.name, target))
        try:
        ...
        for name in p:
            ...
            if plugblacklist and name in plugblacklist.data:
                ...
                continue
            if not event.nolog:
                logging.debug("%s - on demand reloading of %s" % (bot.cfg.name, name))
            try:
        ...
        if done and not event.nolog:
            logging.debug("%s - %s is already loaded" % (bot.cfg.name, str(done)))
        return plugloaded
...
def size():
    return "first: %s - callbacks: %s - last: %s - remote: %s" % (
        first_callbacks.size(),
        callbacks.size(),
        last_callbacks.size(),
        remote_callbacks.size(),
        api_callbacks.size(),
    )
