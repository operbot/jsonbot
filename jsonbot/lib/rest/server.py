# jsonbot/socklib/rest/server.py
#
#
...
from jsonbot.lib.persiststate import ObjectState
...
class RestServerBase(HTTPServer):
    ...
    def addhandler(self, path, type, handler):
        ...
        if splitted not in self.handlers:
    ...
    def handle_error(self, request, addr):
        ...
        logging.warn(
            "rest.server - %s - error %s %s => %s" % (ip, exctype, excvalue, exceptstr)
        )
...
class RestRequestHandler(BaseHTTPRequestHandler):
    ...
    def writeheader(self, type="text/plain"):
        ...
        self.send_header(
            "Content-type", "%s; charset=%s " % (type, sys.getdefaultencoding())
        )
