...
from jsonbot.utils.url import deleteurl, geturl4, posturl, useragent
...
class RestClientAsync(RestClient, asynchat.async_chat):
    ...
    def found_terminator(self):
        ...
        if self.reading_headers:
            ...
            try:
                ...
                logging.warn(
                    "rest.client - %s - can't parse headers %s"
    ...
    def handle_incoming(self):
        ...
        if not self.reading_headers:
            ...
            if self.status >= 400:
                logging.warn(
                    "rest.client - %s - error status: %s" % (self.url, self.status)
                )
                ...
                try:
                    ...
                    logging.info(
                        "rest.client - %s - can't decode %s" % (self.url, self.buffer)
                    )
                    result.error = str(ex)
                except Exception as ex:
            ...
            for cb in self.callbacks:
                try:
                    cb(self, result)
                    logging.info(
                        "rest.client - %s - called callback %s" % (self.url, str(cb))
                    )
                except Exception as ex:
                    handle_exception()
            self.buffer = ""
    ...
    def dorequest(self, method, path, postdata={}, headers={}):
        ...
        if method == "POST":
            s = toenc(
                "%s %s HTTP/1.0\r\n%s\r\n%s\r\n\r\n"
                % (method, path, headerstxt, postdata),
                "ascii",
            )
        else:
