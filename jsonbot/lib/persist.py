...
from jsonbot.lib.errors import JSONParseError, MemcachedCounterError
...
from jsonbot.utils.timeutils import elapsedstring
from jsonbot.utils.trace import calledfrom, callstack, where, whichmodule
...
def cleanup(bot=None, event=None):
    ...
    for p in todo:
        try:
            p.dosave()
            r.append(p)
            logging.warn("saved on retry - %s" % p.fn)
        except (OSError, IOError) as ex:
            logging.error("failed to save %s - %s" % (p, str(ex)))
    for p in r:
        try:
            needsaving.remove(p)
        except ValueError:
...
try:
    ...
    class Persist(object):
        ...
        def __init__(self, filename, default={}, type="cache"):
            ...
            try:
                self.mcounter = int(self.mcounter)
            except ValueError:
                logging.warn(
                    "can't parse %s mcounter, setting to zero: %s"
                    % (self.fn, self.mcounter)
                )
                self.mcounter = 0
            self.data = None
            self.type = type
        ...
        def init(self, default={}, filename=None):
            ...
            if self.jsontxt is None:
                ...
                if self.jsontxt:
                    ...
                    if incr:
                        try:
                            self.mcounter = self.counter = int(incr)
                        except ValueError:
                            logging.error("can't make counter out of %s" % incr)
                    else:
                        self.mcounter = 1
            ...
            logging.warn(
                "*%s* - loaded %s (%s)" % (self.cachetype, self.fn, len(self.jsontxt))
            )
        ...
        def checkmc(self):
            try:
                ...
                self.mcounter = 0
            logging.warn(
                "mcounter for %s is %s (%s)" % (self.fn, self.mcounter, self.counter)
            )
            if (self.mcounter - self.counter) < 0:
                return True
            elif (self.mcounter - self.counter) > 0:
                return True
            return False
        ...
        def save(self):
            ...
            try:
                ...
                self.sync()
                if self not in needsaving:
                    needsaving.appendleft(self)
    ...
    if True:
        got = False
        from jsonbot.memcached import getmc
        ...
        if mc:
            ...
            if status:
                logging.warn(
                    "memcached uptime is %s" % elapsedstring(status[0][1]["uptime"])
                )
                got = True
        if got == False:
    ...
    class Persist(object):
        ...
        def __init__(self, filename, default=None, init=True, postfix=None):
            """Persist constructor"""
            if postfix:
                self.fn = str(filename.strip()) + str("-%s" % postfix)
            else:
                self.fn = str(filename.strip())
        ...
        def init(self, default={}, filename=None):
            ...
            try:
                if self.jsontxt:
                    ...
                    try:
                        self.data = json.loads(str(self.jsontxt))
                    except Exception as ex:
                        logging.error("couldn't parse %s" % self.jsontxt)
                        self.data = None
                        self.dontsave = True
                if not self.data:
                    self.data = LazyDict()
        ...
        def sync(self):
            logging.debug("syncing %s" % self.fn)
            if got:
                mc.set(self.fn, json.dumps(self.data))
            set(self.fn, self.data)
            return self
        ...
        def dosave(self):
            ...
            try:
                if self.dontsave:
                    logging.error("dontsave is set on  %s - not saving" % self.fn)
                    return
                fn = self.fn
                if got:
                    self.mcounter = int(mc.incr(self.countername))
                if got and (self.mcounter - self.counter) > 1:
                    tmp = json.loads(mc.get(fn))
                    if tmp:
                        try:
                            tmp.update(self.data)
                            self.data = LazyDict(tmp)
                            logging.warn("updated %s" % fn)
                ...
                logging.error("not saving %s: %s" % (self.fn, str(ex)))
                raise
            except:
                raise
            finally:
    ...
    def findfilenames(target, filter=[], skip=[]):
        ...
        for f in os.listdir(target):
            ...
            if os.path.isdir(fname):
                res.extend(findfilenames(fname, skip))
            go = True
            for fil in filter:
                if fil not in fname.lower():
                    go = False
...
class GlobalPersist(Persist):
    ...
    def __init__(self, filename, default={}, *args, **kwargs):
        if not filename:
            raise Exception("filename not set in GlobalPersist")
        logging.warn("filename is %s" % filename)
        Persist.__init__(
            self,
            getdatadir() + os.sep + "globals" + os.sep + stripname(filename),
            default=default,
            *args,
            **kwargs
        )
    ...
    def __init__(self, path, *args, **kwargs):
        ...
        for p in path.split(os.sep):
            ...
            try:
                ...
                if "Errno 13" in str(ex) or "Errno 2" in str(ex):
                    continue
                logging.warn("can't make %s - %s" % (pp, str(ex)))
                continue
...
class PlugPersistCollection(PersistCollection):
    def __init__(self):
        plugname = calledfrom(sys._getframe())
        self.path = (
            getdatadir() + os.sep + "plugs" + os.sep + stripname(plugname) + os.sep
        )
