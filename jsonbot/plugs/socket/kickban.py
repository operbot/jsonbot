# jsonbot/plugs/socket/kickban.py
#
#
...
def get_bans(bot, channel):
    ...
    try:
        res = queue368.get(1, 2)
    except queue.Empty:
        pass
    try:
        return bans[bot.cfg.name][channel]
    except KeyError:
        return []
...
def handle_ban_list(bot, ievent):
    ...
    if not banslist:
        ...
        ievent.reply("bans on %s: " % ievent.channel, banslist)
...
def handle_kickban_add(bot, ievent):
    ...
    userhost = getwho(bot, ievent.args[0])
    reason = (
        len(ievent.args) > 1 and " ".join(ievent.args[1:]) or "Permban requested, bye"
    )
    if userhost:
        host = userhost.split("@")[-1].lower()
        if host == get_bothost(bot):
