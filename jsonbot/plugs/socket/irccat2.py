# jsonbot/plugs/socket/irccat2.py
#
#
        ...
        try:
            ...
            for chan in dest:
                ...
                for botname in fleet.list():
                    if botname not in cfg.botnames:
                        continue
...
def init_threaded():
    global server
    if server:
        logging.warn("irccat2 server is already running.")
        return
    if not cfg.enable:
        logging.warn("irccat2 is not enabled.")
        return
    time.sleep(2)
...
def handle_irccat2_add_alias(bot, ievent):
    if len(ievent.args) != 1:
        ievent.reply(
            "syntax: irccat2_add_alias <alias> (where <alias> is the channel you want notifications for)"
        )
        return
    dest = ievent.args[0]
    if not cfg.aliases:
    ...
    cfg.save()
    ievent.reply(
        "%s will now receive irccat2 messages directed at %s" % (ievent.channel, dest)
    )
...
def handle_irccat2_list_aliases(bot, ievent):
    ...
    aliases = [
        dest for dest, chanlist in cfg["aliases"].items() if ievent.channel in chanlist
    ]

    ievent.reply(
        "%s is receiving irccat2 messages directed at: %s"
...
def handle_irccat2_del_alias(bot, ievent):
    if len(ievent.args) != 1:
        ievent.reply(
            "syntax: irccat2_del_alias <alias> (where <alias> is the channel you no longer want notifications for)"
        )
        return
    dest = ievent.args[0]
    ...
    ievent.reply(
        "%s will no longer receive irccat2 messages directed at %s"
        % (ievent.channel, dest)
    )
    cfg.save()
