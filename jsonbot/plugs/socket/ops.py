...
def handle_op1(bot, ievent):
    ...
    if (ievent.user and "OPER" in ievent.user.data.perms) or (
        ievent.chan and userhost in ievent.chan.data.ops
    ):
        bot.doop(chan, nick)
    ievent.done()
...
def handle_permadd(bot, ievent):
    if not ievent.chan:
        ievent.reply("channel is not set in event")
        return
    try:
        nick = ievent.args[0]
    except IndexError:
    ...
    if not userhost in ievent.chan.data.ops:
        ...
        ievent.reply("%s is already in permops list" % userhost)
...
def handle_permdel(bot, ievent):
    ...
    if userhost in ievent.chan.data.ops:
        ...
        ievent.reply("%s is not in permops list" % userhost)
...
def checkmode(bot, ievent):
    ...
    try:
        ...
        logging.warn("unknow mode string format: %s" % str(ievent))
        return
    if "no-op" not in bot.state:
        bot.state["no-op"] = []
        bot.state.save()
    for i in modestr:
        if i == "+":
