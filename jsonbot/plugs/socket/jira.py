...
def getJiraIssueMessage(s, ticket):
    ...
    if info:
        ...
        data = (
            info["key"],
            s["serverInfo"]["priorityMap"][info["priority"]],
            s["serverInfo"]["statusMap"][info["status"]],
            s["serverInfo"]["baseUrl"],
            info["key"],
        )
...
def containsJiraTag(bot, ievent):
    if ievent.how == "backgound":
        return 0
    if not cfg.data.servers:
        return 0
    prefixList = set()
...
def doLookup(bot, ievent):
    if not cfg.data.servers:
        logging.warn("servers is not defined in config.")
        return 0
    prefixList = set()
    serversForPrefix = {}
    for server, serverData in cfg.data["servers"].items():
    ...
    if fnd:
        ...
        prefix = fnd.group(2)
        if not ticket:
            logging.warn("ticket missing: %s" % str(fnd))
            return
        if not prefix:
            logging.warn("prefix missing: %s" % str(fnd))
            return
...
def handle_add_jira_server(bot, ievent):
    ...
    if len(ievent.args) != 4:
        ievent.reply(
            "syntax: add_jira_server [server name] [url] [username] [password]"
        )
        return
...
def handle_jira_issue_lookup_enable(bot, ievent):
    ...
    cfg.save()
    ievent.reply(
        "enabled lookups of %s-* jira tickets in this channel on server %s"
        % (prefix, serverName)
    )
...
examples.add(
    ...
    "enable lookups of jira tickets in the channel",
...
def handle_jira_issue_lookup_disable(bot, ievent):
    ...
    if ievent.channel in server["channels"]:
        ...
        ievent.reply(
            "disabled lookups of jira tickets on server %s from this server"
            % serverName
        )
