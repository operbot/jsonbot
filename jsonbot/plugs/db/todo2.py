...
from jsonbot.lib.datadir import datadir
...
from jsonbot.utils.generic import getwho
from jsonbot.utils.locking import lockdec
from jsonbot.utils.timeutils import striptime, strtotime, today
...
class TodoItem:
    ...
    def __init__(
        self, name, descr, ttime=None, duration=None, warnsec=None, priority=None, num=0
    ):
        self.name = name
        self.time = ttime
...
class TodoDb(object):
    ...
    def get(self, name):
        ...
        name = name.lower()
        result = db.execute(
            """ SELECT * FROM todo WHERE name = %s ORDER BY priority DESC, indx ASC """,
            name,
        )
        res = []
        if result:
    ...
    def setprio(self, who, todonr, prio):
        ...
        if not db:
            ...
            return 0
        result = db.execute(
            """ UPDATE todo SET priority = %s WHERE indx = %s """, (prio, todonr)
        )
        return result
    ...
    def getprio(self, who, todonr):
        ...
        if not db:
            ...
            return []
        result = db.execute(
            """ SELECT name, priority FROM todo WHERE indx = %s """, todonr
        )
        return result
    ...
    def settime(self, who, todonr, ttime):
        ...
        if not db:
            ...
            return 0
        result = db.execute(
            """ UPDATE todo SET time = %s WHERE indx = %s """, (ttime, todonr)
        )
        return result
    ...
    def add(self, name, txt, ttime, alarmnr=None):
        ...
        if ttime:
            if not alarmnr:
                result = db.execute(
                    """ INSERT INTO todo(name, time, descr) VALUES (%s, %s, %s) """,
                    (name, ttime, txt),
                )
            else:
                result = db.execute(
                    """ INSERT INTO todo(name, time, descr, warnsec) VALUES (%s, %s, %s, %s) """,
                    (name, ttime, txt, 0 - alarmnr),
                )

        else:
            result = db.execute(
                """ INSERT INTO todo(name, descr) VALUES (%s, %s) """, (name, txt)
            )
        return result
    ...
    def delete(self, name, indexnr):
        ...
        try:
            ...
            if warnsec:
                alarmnr = 0 - warnsec
                if alarmnr > 0:
                    alarms = plugs.get("jsonbot.plugs.common.alarm")
                    if alarms:
                        alarms.alarms.delete(alarmnr)
        except (IndexError, TypeError):
            pass
        result = db.execute(
            """ DELETE FROM todo WHERE name = %s AND indx = %s """, (name, indexnr)
        )
        return result
    ...
    def toolate(self, name):
        ...
        now = time.time()
        result = db.execute(
            """ SELECT * FROM todo WHERE name = %s AND time < %s """, (name, now)
        )
        res = []
        if result:
            for i in result:
    ...
    def withintime(self, name, time1, time2):
        ...
        name = name.lower()
        result = db.execute(
            """ SELECT * FROM todo WHERE name = %s AND time > %s AND time < %s """,
            (name, time1, time2),
        )
        res = []
        if result:
...
examples.add(
    "t-chandone",
    "t-chandone <listofnrs> .. remove item from channel todo list",
    "t-chandone 2",
)
...
def handle_settodo(bot, ievent):
    ...
    if not ttime is None:
        ievent.reply("time detected " + time.ctime(ttime))
        what = striptime(what)
        karma = plugs.get("jsonbot.plugs.db.karma2")
        if karma:
...
def handle_tomorrow(bot, ievent):
    ...
    if ievent.rest:
        ...
        return
    todoos = todo.withintime(
        username, today() + 24 * 60 * 60, today() + 2 * 24 * 60 * 60
    )
    saytodo(bot, ievent, todoos)
...
examples.add(
    "t-settime",
    "t-settime [<channel|name>] <itemnr> <timestring> .. set todo time",
