...
class InfoItemsDb(object):
    ...
    def add(self, item, description, userhost, ttime):
        ...
        item = item.lower()
        result = db.execute(
            """ INSERT INTO infoitems(item, description, userhost, time) VALUES(%s, %s, %s, %s) """,
            (item, description, userhost, ttime),
        )
        return result
    ...
    def get(self, item):
        ...
        item = item.lower()
        result = db.execute(
            """ SELECT description FROM infoitems WHERE item = %s """, item
        )
        res = []
        if result:
            for i in result:
    ...
    def deltxt(self, item, txt):
        ...
        if not db:
            ...
            return []
        result = db.execute(
            """ DELETE FROM infoitems WHERE item = %s AND description LIKE %s """,
            (item, "%%%s%%" % txt),
        )
        return result
    ...
    def searchitem(self, search):
        ...
        if not db:
            ...
            return []
        result = db.execute(
            """ SELECT item, description FROM infoitems WHERE item LIKE %s """,
            "%%%s%%" % search,
        )
        return result
    ...
    def searchdescr(self, search):
        ...
        if not db:
            ...
            return []
        result = db.execute(
            """ SELECT item, description FROM infoitems WHERE description LIKE %s """,
            "%%%s%%" % search,
        )
        return result
...
def infopre(bot, ievent):
    ...
    if (
        ievent.iscmnd()
        and (ievent.txt and ievent.txt[-1] == "?")
...
def infocb(bot, ievent):
    ...
    if users.allowed(ievent.userhost, "USER"):
        ...
        if data:
            ievent.reply("%s is: " % ievent.txt, data)
...
def handle_question(bot, ievent):
    ...
    if infoitems:
        ievent.reply("%s is: " % what, infoitems)
    else:
