...
class KarmaDb(object):
    ...
    def addwhy(self, item, updown, reason):
        ...
        item = item.lower()
        result = db.execute(
            """ INSERT INTO whykarma(item, updown, why) VALUES (%s, %s, %s) """,
            (item, updown, reason),
        )
        return result
    ...
    def upitem(self, item, reason=None):
        ...
        try:
            ...
            result = db.execute(
                """ INSERT INTO karma(item, value) VALUES (%s, 1) """, item
            )
            if reason:
        ...
        val += 1
        result = db.execute(
            """ UPDATE karma SET value = %s WHERE item = %s """, (val, item)
        )
        if reason:
            self.addwhy(item, "up", reason.strip())
        return result
    ...
    def whykarmaup(self, item):
        ...
        item = item.lower()
        result = db.execute(
            """ SELECT why FROM whykarma WHERE item = %s AND updown = 'up' """, item
        )
        res = []
        if result:
            for i in result:
    ...
    def whykarmadown(self, item):
        ...
        item = item.lower()
        result = db.execute(
            """ SELECT why FROM whykarma WHERE item = %s AND updown = 'down' """, item
        )
        res = []
        if result:
            for i in result:
    ...
    def setwhoup(self, item, nick):
        ...
        nick = nick.lower()
        result = db.execute(
            """ INSERT INTO whokarma(item, nick, updown) VALUES(%s, %s, %s) """,
            (item, nick, "up"),
        )
        return result
    ...
    def getwhoup(self, item):
        ...
        item = item.lower()
        result = db.execute(
            """ SELECT nick FROM whokarma WHERE item = %s AND updown = 'up' """, item
        )
        res = []
        if result:
            for i in result:
    ...
    def getwhodown(self, item):
        ...
        item = item.lower()
        result = db.execute(
            """ SELECT nick FROM whokarma WHERE item = %s AND updown = 'down' """, item
        )
        res = []
        if result:
            for i in result:
    ...
    def search(self, item):
        ...
        item = item.lower()
        result = db.execute(
            """ SELECT item,value FROM karma WHERE item LIKE %s """, "%%%s%%" % item
        )
        res = []
        if result:
            for i in result:
    ...
    def whatup(self, nick):
        ...
        statdict = StatDict()
        result = db.execute(
            """ SELECT item FROM whokarma WHERE nick = %s AND updown = 'up' """, nick
        )
        if not result:
            return []
        for i in result:
    ...
    def whatdown(self, nick):
        ...
        statdict = StatDict()
        result = db.execute(
            """ SELECT item FROM whokarma WHERE nick = %s AND updown = 'down' """, nick
        )
        if not result:
            return []
        for i in result:
    ...
    try:
        ...
        if bot.state[name]["karma"]["count"] > limit:
            if name in ratelimited:
                return 0
            ievent.reply(
                "karma limit reached, you'll have to wait %s seconds"
                % int((bot.state[name]["karma"]["time"] + waittime) - time.time())
            )
...
cmnds.add(
    "^(.+)\+\+\s+#(.*)$", handle_karmaup, ["USER", "KARMA", "ANONKARMA"], regex=True
)
examples.add(
    "++",
    "<item>++ ['#' <reason>] .. higher karma of item with 1 (use optional reason)",
    "1) jsonbot++ 2) jsonbot++ # top bot",
)
...
cmnds.add(
    "^(.+)\-\-\s+#(.*)$", handle_karmadown, ["USER", "KARMA", "ANONKARMA"], regex=True
)
examples.add(
    "--",
    "<item>-- ['#' <reason>] .. lower karma of item with 1 (use optional reason)",
    "1) fuckbot-- 2) fuckbot-- # bad bot",
)
...
def handle_karmawhyup(bot, ievent):
    ...
    if result:
        ievent.reply("whykarmaup of %s: " % item, result)
    else:
...
def handle_whykarmadown(bot, ievent):
    ...
    if result:
        ievent.reply("whykarmadown of %s: " % item, result)
    else:
