...
def handle_meet(bot, ievent):
    ...
    if username:
        ievent.reply(
            "we already have a user with userhost %s (%s)" % (userhost, username)
        )
        return
    result = 0
    name = stripname(nick.lower())
    result = bot.users.add(
        name,
        [
            userhost,
        ],
...
def handle_merge(bot, ievent):
    ...
    name = name.lower()
    if bot.users.gotperm(name, "OPER") and not bot.users.allowed(
        ievent.userhost, "OPER"
    ):
...
examples.add(
    "user-import",
    "user-import <userhost> .. merge record with \
<name> with userhost from the person giving the command (self merge)",
...
examples.add(
    "user-undel",
    "user-del <name> .. undelete user with <username>",
    "user-undel dunker",
)
...
examples.add(
    "user-delperm",
    "delete from user <name> permission <perm>",
...
examples.add(
    "user-adduserhost",
    "user-adduserhost <name> <userhost>",
...
def handle_userhosts(bot, ievent):
    ...
    if userhosts:
        ...
        ievent.reply("no userhosts found")
...
def handle_email(bot, ievent):
    ...
    if len(ievent.args) != 0:
        ievent.reply(
            "use getemail to get the email address of an user .. email shows your own mail address"
        )
        return
...
def handle_permit(bot, ievent):
    ...
    if ievent.rest:
        ievent.reply(
            "use the user-addpermit command to allow somebody something .. use getname <nick> to get the username of somebody .. this command shows what permits you have"
        )
        return
    name = bot.users.getname(ievent.auth)
    if not name:
...
def handle_check(bot, ievent):
    ...
    status = bot.users.getuserstatuses(name)
    ievent.reply(
        "userrecord of %s = userhosts: %s perms: %s email: %s permits: %s status: %s"
        % (name, str(userhosts), str(perms), str(email), str(permits), str(status))
    )
...
def handle_show(bot, ievent):
    ...
    ievent.reply(
        "userrecord of %s = userhosts: %s perms: %s email: %s permits: %s status: %s"
        % (name, userhosts, perms, email, permits, status)
    )
...
def handle_match(bot, ievent):
    ...
    ievent.reply(
        "userrecord of %s = userhosts: %s perms: %s email: %s permits: %s status: %s"
        % (userhost, userhosts, perms, email, permits, status)
    )
