...
def handle_all(bot, event):
    ...
    if bot.type == "irc":
        ...
        printto = event.userhost
    if event.msg and bot.type == "irc":
        target = event.nick
    else:
    ...
    if res:
        ...
        if bot.type in ["xmpp", "sxmpp"]:
            bot.saynocb(
                printto, "lines from %s" % target, res, event=event, showall=True
            )
        else:
            for i in res:
                bot.outnocb(printto, i, event=event, showall=True)
    else:
        event.reply("no data in outputcache of %s (%s)" % (event.channel, bot.cfg.name))
