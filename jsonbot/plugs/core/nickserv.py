# jsonbot/plugs/core/nickserv.py
#
#
...
class NSAuth(Pdod):
    ...
    def add(self, bot, **kwargs):
        ...
        for key in list(options.keys()):
            Pdod.set(self, bot.cfg.name, key, options[key])
        self.save()
    ...
    def register(self, bot, passwd):
        ...
        if bot.cfg.name in self and self.has_key2(bot.cfg.name, "nickserv"):
            bot.sendraw(
                "PRIVMSG %s :%s %s"
                % (self.get(bot.cfg.name, "nickserv"), "REGISTER", passwd)
            )
    ...
    def identify(self, bot):
        ...
        if bot.cfg.name in self:
            ...
            bot.outnocb(
                self.get(bot.cfg.name, "nickserv"),
                "%s %s"
                % (
                    self.get(bot.cfg.name, "identify"),
                    self.get(bot.cfg.name, "password"),
                ),
                how="msg",
            )
