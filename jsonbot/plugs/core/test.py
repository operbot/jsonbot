...
from jsonbot.lib.errors import NoSuchCommand
from jsonbot.lib.eventbase import EventBase
from jsonbot.lib.examples import examples
from jsonbot.lib.runner import cmndrunner, defaultrunner
from jsonbot.lib.threads import start_new_thread
from jsonbot.lib.users import users
from jsonbot.utils.exception import (exceptionevents, exceptionlist, exceptionmsg,
                                 handle_exception)
...
def dotest(bot, event, filter="", direct=False):
    ...
    if True:
        ...
        for example in examplez:
            ...
            teller += 1
            e = bot.make_event(
                event.auth, event.channel, ";" + example, cbtype=event.cbtype
            )
            e.speed = teller % 5
            try:
                if direct:
    ...
    if errors:
        ...
        for cmnd, error in errors.items():
...
examples.add(
    "test-forcedexception", "throw an exception as test", "test-forcedexception"
)
...
cmnds.add(
    ...
    handle_forcedexceptionthreaded,
    "TEST",
    threaded=True,
)
examples.add(
    "test-forcedexceptionthreaded",
    "throw an exception as test in a thread",
    "test-forcedexceptionthreaded",
)
...
def handle_testdocmnd(bot, ievent):
    ...
    if ievent.rest:
        bot.docmnd(
            ievent.origin or ievent.userhost, ievent.channel, ievent.rest, event=ievent
        )
    else:
        ievent.missing("<cmnd>")
        return
...
examples.add(
    "test-deadline",
    "sleep 40 sec to trigger deadlineexceeded exception (GAE)",
    "test-deadline",
)
...
def handle_testhtml(bot, ievent):
    ...
    if not ievent.rest:
        data = '<span style="font-family: fixed; font-size: 10pt"><b>YOOOO BROEDERS</b></span>'
    else:
        data = ievent.rest
