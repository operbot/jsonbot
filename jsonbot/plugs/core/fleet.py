...
from jsonbot.lib.fleet import FleetBotAlreadyExists, getfleet
...
examples.add("fleet-add", "add a fleet bot", "fleet-add local irc localhost jsonbottest")
...
def fleet_cmnd(bot, ievent):
    ...
    try:
        (name, channel, cmndtxt) = ievent.rest.split(" ", 2)
    except ValueError:
        ievent.missing("<botname> <channel> <cmndstring>")
        return
    fleet = getfleet()
    if name.startswith("@"):
        name = name[1:]
        ...
        power = False
    if not cmndtxt.startswith(";"):
        cmndtxt = ";" + cmndtxt
    if name == "all":
        for b in fleet.bots:
            e = b.make_event(ievent.userhost, channel, cmndtxt)
            if not e:
                ievent.reply("can't make event on %s bot (%s)" % (b.cfg.name, b.type))
                continue
            if not power:
                e.nooutput = True
            e.nodispatch = False
...
examples.add(
    "fleet-cmnd",
    "run cmnd on fleet bot(s)",
    "1) fleet-cmnd default-irc uptime 2) fleet-cmnd all uptime",
)
...
examples.add(
    "fleet-save", "save fleet data and all bots that are on the fleet", "fleet-save"
)
