...
def handle_aliasmakeglobal(bot, ievent):
    ...
    if not aliases:
        ievent.chan.data.aliases = aliases = {}
    from jsonbot.lib.aliases import getaliases, savealiases
...
examples.add(
    "alias-makeglobal",
    "push channel specific aliases into the global aliases",
    "alias-makeglobal",
)
...
def handle_aliasget(bot, ievent):
    ...
    aliases = ievent.chan.data.aliases
    if aliases:
        ievent.reply("channel aliases: %s" % str(aliases))
    from jsonbot.lib.aliases import getaliases

    aliases = getaliases()
    if aliases:
        ievent.reply("global aliases: %s" % aliases.tojson())
