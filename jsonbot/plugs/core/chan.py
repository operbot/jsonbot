...
def handle_chantoken(bot, event):
    ...
    if not bot.isgae:
        event.reply("this command only works on the App Engine")
        return
    import google
...
examples.add(
    "chan-token", "create a new token for the appengine channel API", "chan-token"
)
...
examples.add(
    "chan-withnotice",
    "make bot use notice on channel the command was given in",
    "chan-withnotice",
)
...
examples.add(
    "chan-withprivmsg",
    "make bot use privmsg on channel command was given in",
    "chan-withprivmsg",
)
...
def handle_chandenyplug(bot, event):
    ...
    if not event.rest in event.chan.data.denyplug:
        ...
        event.reply(
            "%s is already being denied in channel %s" % (event.rest, event.channel)
        )
...
examples.add(
    "chan-denyplug",
    "deny a plugin command or callbacks to be executed in a channel",
    "chan-denyplug idle",
)
...
def handle_chanallowplug(bot, event):
    ...
    if event.rest in event.chan.data.denyplug:
        ...
        event.reply(
            "%s is already being allowed in channel %s" % (event.rest, event.channel)
        )
...
examples.add(
    "chan-allowplug",
    "allow a plugin command or callbacks to be executed in a channel",
    "chan-denyplug idle",
)
...
def handle_chanallowcommand(bot, event):
    ...
    try:
        ...
        event.missing("<cmnd>")
        return
    if not cmnd in event.chan.data.allowcommands:
        event.chan.data.allowcommands.append(cmnd)
        event.chan.save()
        event.done()
...
examples.add(
    "chan-allowcommand",
    "add a command to the allow list. allows for all users.",
...
def handle_chansilentcommand(bot, event):
    ...
    try:
        ...
        event.missing("<cmnd>")
        return
    if not cmnd in event.chan.data.silentcommands:
        event.chan.data.silentcommands.append(cmnd)
        event.chan.save()
        event.done()
...
def handle_chanloudcommand(bot, event):
    ...
    try:
        cmnd = event.args[0]
        event.chan.data.silentcommands.remove(cmnd)
        event.chan.save()
        event.done()
    except (IndexError, ValueError):
        event.reply("%s is not in the silencelist" % event.rest)


cmnds.add(
    "chan-loudcommand",
    handle_chanloudcommand,
    [
        "OPER",
    ],
)
examples.add(
    "chan-loudcommand",
    "remove a command from the silence list.",
...
def handle_chanremovecommand(bot, event):
    ...
    try:
        cmnd = event.args[0]
        event.chan.data.allowcommands.remove(cmnd)
        event.chan.save()
        event.done()
    except (IndexError, ValueError):
        event.reply("%s is not in the whitelist" % event.rest)


cmnds.add(
    "chan-removecommand",
    handle_chanremovecommand,
    [
        "OPER",
    ],
)
...
def handle_chanupgrade(bot, event):
    ...
    if prev.data:
        ...
        if prev.data:
            ...
            event.reply("can't find previous channel data")
...
def handle_chanallowwatch(bot, event):
    ...
    if not event.rest:
        event.missing("<JID/channel>")
        return
    if event.rest not in event.chan.data.allowwatch:
        event.chan.data.allowwatch.append(event.rest)
        event.chan.save()
    event.done()
...
examples.add(
    "chan-allowwatch",
    "allow channel events to be watch when forwarded",
...
def handle_chandelwatch(bot, event):
    ...
    if not event.rest:
        ...
        return
    try:
        event.chan.data.allowwatch.remove(event.rest)
        event.chan.save()
    except:
        event.reply("%s is not in the allowout list" % event.rest)
        return
    event.done()
...
examples.add(
    "chan-delwatch",
    "deny channel events to be watched when forwarded",
...
examples.add(
    "chan-enable",
    "enable a channel (allow for handling of events concerning this channel (convore for now)",
    "chan-enable",
)
...
examples.add(
    "chan-disable",
    "disable a channel (disallow for handling of events concerning this channel (convore for now)",
    "chan-disable",
)
