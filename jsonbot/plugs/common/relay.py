...
from jsonbot.lib.callbacks import first_callbacks
from jsonbot.lib.commands import cmnds
from jsonbot.lib.errors import NoSuchWave
...
from jsonbot.utils.generic import stripcolor
...
# see jsonbot/persist/persist.py
...
def relayprecondition(bot, event):
    ...
    if event.isrelayed:
        ...
        return False
    if event.forwarded:
        logging.info("%s forarded" % bot.cfg.name)
        return False
    txt = event.txt.strip()
    if txt:
        txt = txt.lower()
        if (
            ...
            and bot.cfg.nick
            and (
                txt.startswith("[%s]" % event.nick.lower())
                or txt.find("[%s]" % bot.cfg.nick.lower()) != -1
            )
        ):
            return False
    ...
    try:
        ...
        for botname, type, target in relay.data[origin]:
            try:
                ...
                if outbot:
                    logging.info(
                        "outbot found - %s - %s" % (outbot.cfg.name, outbot.type)
                    )
                    ...
                    logging.info(
                        "can't find bot for (%s,%s,%s)" % (botname, type, target)
                    )
...
def handle_relay(bot, event):
    ...
    try:
        ...
        try:
            ...
            try:
                botname = bot.cfg.name
                type = bot.cfg.bottype
                target = event.args[0]
            except IndexError:
                event.missing("[<botname>] [<bottype>] <target>")
                return
    origin = str((bot.cfg.name, event.channel))
    if not getfleet().byname(botname):
        event.reply("no %s bot in fleet." % botname)
        return
    if origin not in relay.data:
        relay.data[origin] = []
    try:
...
examples.add(
    "relay",
    "open a relay to another user/channel",
...
def handle_relayblock(bot, event):
    ...
    try:
        ...
        event.missing("<bottype> <target>")
        return
    origin = str((bot.cfg.name, event.origin or event.channel))
    if origin not in block.data:
        block.data[origin] = []
...
def handle_relayunblock(bot, event):
    ...
    origin = str((bot.cfg.name, event.origin or event.channel))
    try:
        block.data[origin].remove([bot.cfg.name, target])
        block.save()
    except (KeyError, ValueError):
        pass
    event.done()
