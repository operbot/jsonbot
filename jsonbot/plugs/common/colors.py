# jsonbot/plugs/common/colors.py
#
#
...
from jsonbot.lib.morphs import outputmorphs
...
def docolormorph(txt, event):
    ...
    if not txt:
        return txt
    if event and not event.bottype == "irc":
        return txt
    splitted = txt.split()
    for s in splitted:
        for t, color in state.data.colormapping.items():
            try:
                ...
                logging.warn("color - %s is not a digit" % color)
                continue
            if c < 0:
                d = "00"
            elif c < 9:
                ...
                d = "%s" % c
            if t in s:
                txt = txt.replace(s, "\003%s%s\003" % (d, s))
    return txt
...
def handle_coloradd(bot, event):
    ...
    try:
        (txt, color) = event.rest.rsplit(" ", 1)
    except (TypeError, ValueError):
        event.missing("<txt> <color>")
        return
    try:
        c = int(color)
    except (ValueError, TypeError):
        event.reply("color must be between 0 and 9")
        return
    if c < 0:
        c = 0
    if c > 15:
...
def handle_colordel(bot, event):
    ...
    if not event.rest:
        ...
        return
    try:
        del state.data.colormapping[event.rest]
        state.save()
    except KeyError:
        event.reply("we are not morphing %s" % event.rest)
