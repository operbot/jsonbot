# jsonbot/plugs/common/spider.py
#
#
...
from jsonbot.imports import getBeautifulSoup
...
from jsonbot.lib.persist import PersistCollection
from jsonbot.lib.threadloop import ThreadLoop
...
from jsonbot.utils.url import Url, geturl2, striphtml
from jsonbot.utils.urldata import UrlData
...
class Spider(ThreadLoop):
    ...
    def handle(self, event, url, depth, speed=5):
        ...
        if not self.url.base in url:
            logging.warn("skipping %s (%s)" % (url, self.url.base))
            return
        ...
        try:
            if url not in self.urls:
                ...
                content = page.fetch()
                event.reply(
                    "fetched %s - %s - %s" % (url, len(content), content.status)
                )
                try:
...
def handle_spider(bot, event):
    ...
    try:
        ...
        depth = 3
    spider = Spider(url)
    if not spider in running:
        running.append(spider)
    thr = spider.start()
...
examples.add(
    "spider", "run the spider on a site.", "spider http://jsonbot.org/handbook"
)
