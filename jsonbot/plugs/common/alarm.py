...
from jsonbot.lib.nextid import nextid
from jsonbot.lib.periodical import periodical
...
from jsonbot.utils.timeutils import striptime, strtotime
...
class Alarms(Persist):
    ...
    def alarmsay(self, item):
        ...
        if not bot:
            bot = getfleet().makebot(None, item.botname)
        if bot:
            if bot.type == "irc":
                bot.connectok.wait()
            if item.printto:
                bot.say(item.printto, "[%s] %s" % (item.nick, item.txt), speed=1)
            else:
                bot.say(item.nick, item.txt, speed=1)
        else:
            logging.warn("can't find %s bot in fleet" % item.botname)
...
def handle_alarmadd(bot, ievent):
        ...
        try:
            ...
            if bot.type == "xmpp":
                if ievent.groupchat:
                    ...
                    nrid = alarms.add(
                        bot.cfg.name, ievent.stripped, ttime, " ".join(ievent.args[1:])
                    )
            elif bot.type == "irc":
                if ievent.msg:
                    nrid = alarms.add(
    ...
    if time.time() > alarmtime:
        ...
        return
    if bot.jabber:
        nrid = alarms.add(bot.cfg.name, ievent.nick, alarmtime, txt, ievent.channel)
    else:
        nrid = alarms.add(bot.cfg.name, ievent.nick, alarmtime, txt, ievent.channel)
...
def handle_myalarmslist(bot, ievent):
    ...
    if result:
        ievent.reply("your alarms: ", result)
    else:
        ievent.reply("no alarms")
