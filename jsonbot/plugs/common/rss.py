...
from jsonbot.imports import getfeedparser, getjson
...
from jsonbot.lib.persist import Persist, PlugPersist
...
from jsonbot.utils.generic import fromenc, getnick, jsonstring
from jsonbot.utils.lazydict import LazyDict
from jsonbot.utils.pdod import Pdod
from jsonbot.utils.pdol import Pdol
from jsonbot.utils.statdict import StatDict
from jsonbot.utils.url import geturl2, useragent
...
try:
    ...
    from jsonbot.lib.cache import delete, get, set
...
possiblemarkup = {
    "separator": "set this to desired item separator",
    "all-lines": "set this to 1 if you don't want items to be aggregated",
    "tinyurl": "set this to 1 when you want to use tinyurls",
    "skipmerge": "set this to 1 if you want to skip merge commits",
    "reverse-order": "set this to 1 if you want the rss items displayed with oldest item first",
...
if not sleeptime.data:
    sleeptime.data = LazyDict()
    sleeptime.save()
...
class Feed(Persist):
    ...
    def ownercheck(self, userhost):
        ...
        try:
            return self.data.owner.lower() == userhost.lower()
        except KeyError:
            pass
        return False
    ...
    def getdata(self):
        ...
        if result is None:
            ...
            logging.debug("got result from %s *cached*" % strippassword(url))
        return result

    def fetchdata(self, data=None):
        """get data of rss feed."""
        name = self.data.name
        from jsonbot.utils.url import enabled

        if not enabled:
            return []
        logging.info("fetching %s" % name)
        global etags
        if name and name in etags.data:
            etag = etags.data[name]
        else:
            etag = None
        if data:
        ...
        if status == 304:
            return []
        if result:
            set(self.data.url, result.entries, namespace="rss")
        if data:
            try:
                etag = etags.data[name] = data.headers.get("etag")
                logging.info("etag of %s set to %s" % (name, etags.data[name]))
                etags.sync()
            except KeyError:
                etag = None
        else:
            try:
                etag = etags.data[name] = result.etag
                logging.info("etag of %s set to %s" % (name, etags.data[name]))
                etags.sync()
            except (AttributeError, KeyError):
                etag = None
        if not name in urls.data:
            urls.data[name] = self.data.url
            urls.save()
        logging.debug("got result from %s" % strippassword(self.data.url))
        if result and "bozo_exception" in result:
            logging.debug(
                "%s bozo_exception: %s"
                % (strippassword(self.data.url), result["bozo_exception"])
            )
    ...
    def sync(self):
        ...
        if not self.data.running:
            logging.info(
                "%s not enabled .. %s not syncing "
                % (self.data.name, strippassword(self.data.url))
            )
            return False
        ...
        try:
            ...
            for item in loopover:
                ...
                try:
                    ...
                    logging.warn(
                        "%s is not in the format (botname, type, channel)" % str(item)
                    )
                    continue
                if not botname:
                    logging.error("%s - %s is not correct" % (name, str(item)))
                    continue
                if not type:
                    logging.error("%s - %s is not correct" % (name, str(item)))
                    continue
                try:
                    ...
                    if not bot and ongae:
                        bot = getfleet().makebot(type, botname)
                except NoSuchBotType as ex:
                    logging.warn("can't make bot - %s" % str(ex))
                    continue
                if not bot:
                    logging.warn("can't find %s bot in fleet" % botname)
                    continue
                if bot.type == "irc":
                    logging.debug("waiting for irc bot to connect")
                    bot.connectok.wait()
                res2 = datalist
                if type == "irc" and not "#" in channel:
                    nick = getnick(bot, channel)
                    if not nick:
                        logging.error("can't get nick to send to %s" % channel)
                        continue
                else:
                    nick = None
                if self.markup.get(jsonstring([name, type, channel]), "reverse-order"):
                    res2 = res2[::-1]
                if self.markup.get(jsonstring([name, type, channel]), "all-lines"):
                    for i in res2:
                        ...
                        try:
                            bot.say(nick or channel, response, **{"headlines": True})
                        except Exception as ex:
                            handle_exception()
                else:
                    sep = self.markup.get(
                        jsonstring([name, type, channel]), "separator"
                    )
                    if sep:
    ...
    def makeresponse(self, name, type, res, channel, sep=" || "):
        ...
        if self.markup.get(jsonstring([name, type, channel]), "nofeedname"):
            result = ""
        else:
            result = "<b>[%s]</b> - " % name
        try:
            itemslist = self.itemslists.data[jsonstring([name, type, channel])]
        except KeyError:
            itemslist = self.itemslists.data[jsonstring([name, type, channel])] = [
                "title",
                "link",
        ...
        for j in res:
            if (
                self.markup.get(jsonstring([name, type, channel]), "skipmerge")
                and "Merge branch" in j["title"]
            ):
                continue
            resultstr = ""
            for i in itemslist:
                try:
                    ...
                    if item.startswith("http://"):
                        if self.markup.get(
                            jsonstring([name, type, channel]), "tinyurl"
                        ):
                            try:
    ...
    def cloneurl(self, url, auth):
        ...
        for line in data.split("\n"):
            try:
                ...
                logging.debug("cloneurl - can't split %s line" % line)
                continue
            if self.byname(name):
                logging.debug("cloneurl - already got %s feed" % name)
                continue
            if url.endswith("<br>"):
                url = url[:-4]
...
class Rsswatcher(Rssdict):
    ...
    def checkfeed(self, url, event):
        ...
        if result and "bozo_exception" in result:
            event.reply(
                "%s bozo_exception: %s" % (strippassword(url), result["bozo_exception"])
            )
            return True
        try:
            status = result.status
            event.reply("%s - status is %s" % (strippassword(url), status))
        except AttributeError:
            status = 200
    ...
    def handle_data(self, data, name=None):
        ...
        try:
            ...
            if rssitem:
                ...
                logging.info("can't find %s item" % strippassword(url))
                del data
                return
            if not name in urls.data:
                urls.data[name] = url
            ...
            if res:
                rssitem.deliver(res, save=True)
            else:
                logging.info("%s - no items to deliver" % name)
        except Exception as ex:
            handle_exception(txt=name)
        del data
        return True
    ...
    def getall(self):
        ...
        for name in self.data["names"]:
            self.feeds[name] = Feed(name)
        return self.feeds

    def shouldpoll(self, name, curtime):
        """check whether poll of feed in needed."""
        feed = self.byname(name)
        if feed and not feed.data.stoprunning and feed.data.running:
            return shouldpoll(name, curtime)
        ...
        if feed:
            ...
            if result:
                ...
                if res2:
                    ...
                    logging.debug("no result from check - %s" % name)
            else:
                logging.debug("not result from sync - %s" % name)
        else:
            logging.warn("can't find %s feed" % name)
    ...
    def getfeeds(self, botname, type, channel):
        ...
        for name in self.runners():
            ...
            if not z or not z.data.running:
                continue
            if (
                jsonstring([botname, type, channel]) in z.data.watchchannels
                or [botname, type, channel] in z.data.watchchannels
            ):
    ...
    def start(self, botname, bottype, name, channel):
        ...
        target = channel
        if (
            not jsonstring([botname, bottype, target]) in rssitem.data.watchchannels
            and not [botname, bottype, target] in rssitem.data.watchchannels
        ):
...
def shouldpoll(name, curtime):
    ...
    global lastpoll
    try:
        lp = lastpoll.data[name]
    except KeyError:
        lp = lastpoll.data[name] = time.time()
        lastpoll.sync()
    global sleeptime
    try:
        st = sleeptime.data[name]
    except KeyError:
        st = sleeptime.data[name] = 900
        sleeptime.sync()
    logging.debug(
        "pollcheck - %s - %s - remaining %s"
        % (name, time.ctime(lp), (lp + st) - curtime)
    )
    if curtime - lp > st:
        return True
...
def rssfetchcb(rpc):
    ...
    try:
        data = rpc.get_result()
    except google.appengine.api.urlfetch_errors.DownloadError as ex:
        logging.warn("%s - error: %s" % (strippassword(rpc.final_url), str(ex)))
        return
    name = rpc.feedname
    ...
    if data.status_code == 200:
        ...
        logging.info(
            "fetch returned status code %s - %s"
            % (data.status_code, strippassword(rpc.feedurl))
        )
...
def doperiodicalGAE(*args, **kwargs):
    ...
    for feed in watcher.data.names:
        if not shouldpoll(feed, curtime):
            continue
    ...
    for f in feedstofetch:
        ...
        if f not in urls.data:
            logging.warn("adding %s to urls" % f)
            url = Feed(f).data.url
            urls.data[f] = url
            got = True
        else:
        ...
        try:
            urlfetch.make_fetch_call(rpc, url, headers={"If-None-Match": etag})
            rpcs.append(rpc)
        except urlfetch_errors.InvalidURLError:
            logging.warn("%s url is invalid" % url)
            continue
        except Exception as ex:
            handle_exception()
    for rpc in rpcs:
        try:
            rpc.wait()
        except google.appengine.api.urlfetch_errors.InvalidURLError as ex:
            logging.warn("url is invalid: %s" % str(ex))
        except Exception as ex:
            handle_exception()
    if feedstofetch:
        lastpoll.save()
...
def handle_rssadd(bot, ievent):
    ...
    if watcher.checkfeed(url, ievent):
        ...
        ievent.reply("%s is not valid" % strippassword(url))


cmnds.add("rss-add", handle_rssadd, "USER")
examples.add(
    "rss-add",
    "add a feed to the rsswatcher",
...
def handle_rssregister(bot, ievent):
    ...
    if watcher.byname(name):
        ievent.reply(
            "we already have a feed with %s name .. plz choose a different name" % name
        )
        return
    if watcher.checkfeed(url, ievent):
        feed = watcher.add(name, url, ievent.userhost)
...
def handle_rsswatch(bot, ievent):
    ...
    try:
        ...
        try:
            ...
            ievent.missing("<feedname> [secondstosleep]")
            return
    try:
    ...
    for feed in target:
        ...
        if name not in sleeptime.data:
            ...
            sleeptime.save()
        try:
            watcher.watch(feed, sleepsec)
        except Exception as ex:
            ievent.reply("%s - %s" % (feed, str(ex)))
            continue
        got.append(feed)
    if got:
        ievent.reply("watcher started ", got)
    else:
        ievent.reply("already watching ", target)


cmnds.add("rss-watch", handle_rsswatch, "USER")
examples.add(
    "rss-watch",
    "start watching a feed",
...
def handle_rssstart(bot, ievent):
    ...
    if feeds[0] == "all":
        feeds = watcher.list()
    if ievent.options and ievent.options.channel:
        target = ievent.options.channel
    else:
    ...
    for chan in started:
        ...
        if name not in tchan.data.feeds:
            tchan.data.feeds.append(name)
            tchan.save()
...
def handle_rssstop(bot, ievent):
    ...
    if not ievent.rest:
        ...
        return
    if ievent.rest == "all":
        loopover = ievent.chan.data.feeds
    else:
        loopover = [
            ievent.rest,
        ]
    ...
    for name in loopover:
        ...
        if not rssitem.data.running:
            continue
        try:
            rssitem.data.watchchannels.remove([bot.cfg.name, bot.type, target])
        except ValueError:
            try:
                rssitem.data.watchchannels.remove([bot.cfg.name, bot.type, target])
            except ValueError:
...
examples.add(
    "rss-stopall",
    "rss-stopall .. stop all rss feeds (per user/channel) ",
    "rss-stopall",
)
...
def handle_rssaddchannel(bot, ievent):
    ...
    if not rssitem.data.running:
        ...
        return
    if (
        jsonstring([botname, type, channel]) in rssitem.data.watchchannels
        or [botname, channel] in rssitem.data.watchchannels
    ):
        ievent.reply(
            "we are already monitoring %s on (%s,%s)" % (name, botname, channel)
        )
        return
...
examples.add(
    "rss-addchannel",
    "add a channel to watchchannels of a feed",
...
def handle_rsssetitems(bot, ievent):
    ...
    try:
        ...
        ievent.missing("<feedname> <list of tokens>")
        return
    if ievent.options and ievent.options.channel:
        target = ievent.options.channel
    else:
...
examples.add(
    "rss-setitems",
    "set tokens of the itemslist (per user/channel)",
...
def handle_rsssleeptime(bot, ievent):
    ...
    try:
        ievent.reply(
            "sleeptime for %s is %s seconds" % (name, str(sleeptime.data[name]))
        )
    except KeyError:
...
examples.add(
    "rss-setsleeptime",
    "set sleeping time of a feed .. min 60 sec",
...
def handle_rssurl(bot, ievent):
    ...
    if not result:
        ievent.reply("don't know url for %s" % name)
        return
...
def handle_rssseturl(bot, ievent):
    ...
    if watcher.seturl(name, url):
        ...
        ievent.reply("failed to set url of %s to %s" % (name, strippassword(url)))
...
def handle_rssitemslist(bot, ievent):
    ...
    if ievent.options and ievent.options.channel:
        ...
        target = ievent.channel
    try:
        itemslist = rssitem.itemslists[jsonstring([name, bot.type, target])]
    except KeyError:
        ievent.reply("no itemslist set for (%s, %s)" % (name, target))
        return
...
examples.add(
    "rss-scan",
    "get possible tokens of a feed that can be displayed ",
    "rss-scan jsonbot",
)
...
def handle_rssfeeds(bot, ievent):
    ...
    if result:
        ievent.reply("feeds running in %s: " % target, result)
    else:
        ievent.reply("%s has no feeds running" % target)
...
def handle_rsslink(bot, ievent):
    ...
    try:
        ...
        ievent.missing("<feedname> <searchtxt>")
        return
...
examples.add(
    "rss-link",
    "give link of feeds which title matches search key",
...
def handle_rssimport(bot, ievent):
    ...
    try:
        ...
        if not data:
            ievent.reply("can't fetch data from %s" % ievent.rest)
    except Exception as ex:
        ievent.reply("error fetching %s: %s" % (ievent.rest, str(ex)))
        return
    try:
        element = etree.fromstring(data)
    except Exception as ex:
        ievent.reply("error reading %s: %s" % (ievent.rest, str(ex)))
        return
    teller = 0
    errors = {}
...
examples.add(
    "rss-import",
    "import rss feeds from a remote OPML file.",
    "rss-import http://jsonbot.org/feeds.opml",
)
