...
def handle_dotour(bot, event):
    if event.user.state.data.notour:
        event.reply("the tour is disabled for %s" % event.userhost)
        return
    event.reply(
        "will say something every 5 seconds. you can disable this tour by typing !set notour 1"
    )
    time.sleep(5)
