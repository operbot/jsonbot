...
def handle_todo(bot, ievent):
    ...
    if len(ievent.args) > 0:
        ...
        return
    if ievent.options and ievent.options.dochan:
        name = ievent.channel
    else:
        name = ievent.user.data.name
    try:
...
examples.add(
    "todo",
    "show todo items or add a todo item, a time can be given as well.",
...
def handle_setpriority(bot, ievent):
    ...
    try:
        ...
        ievent.missing("<itemnr> <priority>")
        return
    try:
        itemnr = int(itemnr)
        prio = int(prio)
    except ValueError:
        ievent.missing("priority needs to be an integer")
        return
    if ievent.options and ievent.options.dochan:
        name = ievent.channel
    else:
...
def handle_todosettime(bot, ievent):
    ...
    try:
        ...
        ievent.missing("<itemnr> <timestring>")
        return
    try:
        itemnr = int(itemnr)
    except ValueError:
        ievent.missing("item number needs to be a string.")
        return
    if ievent.options and ievent.options.dochan:
        name = ievent.channel
    else:
...
def saytodo(bot, ievent, todoos, target=""):
    """output todo items."""
    if not todoos:
        ievent.reply("nothing todo for %s ;]" % target or "you")
        return
    result = []
    now = time.time()
    counter = 1
    ...
    if result:
        ievent.reply("todo for %s: " % target or "you", result, dot=" ")
