...
from jsonbot.lib.users import getusers
from jsonbot.utils.exception import handle_exception
from jsonbot.utils.pdod import Pdod
from jsonbot.utils.url import geturl, geturl2, striphtml, useragent
    ...
    def handle_show(self, bot, ievent):
        if len(ievent.args) != 1:
            try:
                ...
                ievent.missing(
                    "<bug id> -- if you were looking to report a bug, please go to %s"
                    % open_url
                )
            except BugTrackerNotImplemented:
                ievent.missing("<bug id>")
        ...
        try:
            ...
            ievent.reply("no #%s bug found: %s" % (ievent.args[0], str(e)))
            return
        except AssertionError as e:
            ievent.reply("error: %s" % e)
        except BugTrackerNotImplemented:
...
class Bugzilla(BugTracker):
    def __init__(self, url, autoshow):
        ...
        self.bugurls = [re.compile(r"%s/show_bug.*id=(\d+)" % self.url)]
...
class Flyspray(BugTracker):
    def __init__(self, url, autoshow):
        BugTracker.__init__(self, url, autoshow)
        self.bugurls = [re.compile(r"%s/task/(\d+)" % self.url)]
    ...
    def list(self):
        ...
        if bugall:
            for entry in bugall:
                try:
                    bugid = entry.getElementsByTagName("id")[
                        0
...
class GoogleCode(BugTracker):
    def __init__(self, url, autoshow):
        BugTracker.__init__(self, url, autoshow)
        self.bugurls = [re.compile(r"%s/issues.*id=(\d+)" % self.url)]

    def close(self, *args):
        raise BugTrackerNotSupported(
            "google code needs authorization which I don't support"
        )
...
class Trac(BugTracker):
    def __init__(self, url, autoshow):
        BugTracker.__init__(self, url, autoshow)
        self.bugurls = [re.compile(r"%s/ticket/(\d+)" % self.url)]
        ...
        if bugall:
            for item in bugall:
                ...
                if "comment added" in title:
                    try:
                        author = item.getElementsByTagName("dc:creator")[
                            0
                        ].firstChild.nodeValue
                    except IndexError:
                        author = "anonymous"
                    comment = item.getElementsByTagName("description")[
                        0
                    ].firstChild.nodeValue
...
class BugManager(Pdod):
    ...
    def __init__(self):
        Pdod.__init__(
            self,
            os.path.join(
                getdatadir() + os.sep + "plugs" + os.sep + "bugtracker", "bugtracker"
            ),
        )
        self.load()
    ...
    def add(self, bot, ievent, type, url):
        assert type in list(
            self.types.keys()
        ), "unknown bugtracker type, supported: %s" % ", ".join(list(self.types.keys()))
        if hasattr(bot, "name"):
    ...
    def handle_tracker_info(self, bot, ievent):
        if not self.check(bot, ievent):
            ...
            ievent.reply(
                "bugtracker type: %s .. url: %s .. autoshow: %s"
                % (data[1], data[0], str(data[2]))
            )
    ...
    def handle_autoshow(self, bot, ievent):
        if not self.check(bot, ievent):
            ...
            return
        self.data[bot.cfg.name][ievent.channel][2] = not self.data[bot.cfg.name][
            ievent.channel
        ][2]
        try:
            self.instances[bot.cfg.name][ievent.channel].autoshow = self.data[
                bot.cfg.name
            ][ievent.channel][2]
        except:
            pass
        self.save()
        ievent.reply(
            "autoshow is now %s" % str(self.data[bot.cfg.name][ievent.channel][2])
        )
    ...
    def handle_show(self, bot, ievent):
        ...
        try:
            ...
            ievent.reply("key error: %s" % str(ex))
            return False
        except Exception as e:
            handle_exception()
...
examples.add(
    "bug-autoshow", "toggle automatic retrieval of bug information", "bug-autoshow"
)
...
examples.add(
    "bug-set",
    "set a bugtracker for the channel",
    "bug-set googletracker https://code.google.com/feeds/issues/p/jsonbot/issues/full",
)
