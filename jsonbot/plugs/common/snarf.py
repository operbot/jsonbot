...
from jsonbot.utils.url import decode_html_entities, get_encoding, geturl, geturl2
...
re_html_valid = {
    "result": re.compile(
        "(Failed validation, \d+ errors?|Passed validation)", re.I | re.M
    ),
    "modified": re.compile(
        '<th>Modified:</th><td colspan="2">([^<]+)</td>', re.I | re.M
    ),
    "server": re.compile('<th>Server:</th><td colspan="2">([^<]+)</td>', re.I | re.M),
    "size": re.compile('<th>Size:</th><td colspan="2">([^<]+)</td>', re.I | re.M),
    "content": re.compile(
        '<th>Content-Type:</th><td colspan="2">([^<]+)</td>', re.I | re.M
    ),
    "encoding": re.compile(
        '<td>([^<]+)</td><td><select name="charset" id="charset">', re.I | re.M
    ),
    "doctype": re.compile(
        '<td>([^<]+)</td><td><select id="doctype" name="doctype">', re.I | re.M
    ),
}
...
def geturl_title(url):
    ...
    try:
        ...
        logging.warn("URLError %s" % str(ex))
        return False
...
def handle_snarf(bot, ievent, direct=True):
    ...
    if title:
        ...
        ievent.reply("no title found at %s" % urllib.parse.urlparse(url)[1])
...
def handle_validate(bot, ievent):
    ...
    if result:
        ...
        if len(host) > 20:
            host = host[0:20] + "..."
        ievent.reply(
            "%s: %s | modified: %s | server: %s | size: %s | content-type: %s | encoding: %s | doctype: %s"
            % tuple(
                [host]
                + [
                    result[x]
