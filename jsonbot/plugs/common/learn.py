...
from jsonbot.lib.persist import GlobalPersist, PlugPersist
...
examples.add(
    "learn-chan",
    "learn the bot a description of an item. (channel specific)",
...
examples.add(
    "forget-chan",
    "forget a description of an item. (channel specific)",
...
def handle_whatis(bot, event):
    ...
    if result:
        event.reply("%s is " % event.rest, result, dot=", ")
    else:
...
examples.add(
    "learn-toglobal",
    "move channel specific learn data to the global database.",
    "learn-toglobal",
)
...
def prelearn(bot, event):
    ...
    if (
        event.txt
        and (event.txt[0] == "?" or event.txt[-1] == "?")
        and not event.forwarded
    ):
        return True
    return False
...
def learncb(bot, event):
    ...
    if target in items.data:
        result = items.data[target]
    globalitems = GlobalPersist("learndb")
    if target in globalitems.data:
        if not target in result:
            result.extend(globalitems.data[target])
    if result:
        event.reply("%s is " % target, result, dot=", ")
    event.ready()
