# jsonbot/plugs/common/forward.py
#
#
...
from jsonbot.plugs.common.twitter import postmsg
from jsonbot.utils.exception import handle_exception
from jsonbot.utils.generic import stripcolor, strippedtxt
...
def handle_forwarddel(bot, event):
    ...
    try:
        ...
        return
    forward.save()
    if event.rest in event.chan.data.forwards:
        event.chan.data.forwards.remove(event.rest)
        event.chan.save()
    event.done()
...
def handle_forwardlist(bot, event):
    ...
    try:
        event.reply(
            "forwards for %s: " % event.channel, forward.data.channels[event.channel]
        )
    except KeyError:
...
examples.add(
    "forward-stop",
    "stop forwarding a channel to provided JIDS",
    "forward-stop jsonbot@jsonbot.org",
)
