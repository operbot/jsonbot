...
def exceptionmsg():
    ...
    for i in trace:
        ...
        mod = []
        for i in plugfile[::-1]:
            if i in ["jsonbot.upload"]:
                break
            mod.append(i)
            if i in ["jsonbot", "waveapi", "google", "data"]:
                break
        ownname = ".".join(mod[::-1])
...
def handle_exception(event=None, log=True, txt="", stop=False):
    ...
    if log:
        logging.error(RED + txt + " " + errormsg + ENDC)
    if event:
        exceptionevents.append((event, errormsg))
        if event.bot:
            event.bot.error = errormsg
            if event.bot.type == "irc":
                target = event.nick
            else:
                target = event.channel
            if target:
                event.bot.saynocb(
                    target, "*sorry* - an exception occured - %s" % errormsg
                )
    if stop or bork:
        os._exit(1)
