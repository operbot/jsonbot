...
from jsonbot.lib.errors import PropertyIgnored
from jsonbot.utils.exception import handle_exception
from jsonbot.utils.locking import lockdec
from jsonbot.utils.trace import where, whichmodule
...
def dumpelement(element, prev={}, withtypes=False, full=False):
    ...
    try:
        ...
        logging.error("can't copy %s" % str(ex))
    ...
    try:
        ...
        new = LazyDict()
    for name in elem:
        if not full and checkignore(name, defaultignore):
            continue
        elif checkignore(name, minignore):
            continue
        try:
            json.dumps(elem[name])
            try:
                new[name] = stripignore(elem[name])
            except:
                new[name] = elem[name]
        except TypeError:
            if type(elem[name]) not in jsontypes:
                if withtypes:
                    new[name] = str(type(elem[name]))
            else:
...
class LazyDict(dict):
    ...
    def __setattr__(self, attr, value):
        ...
        if (
            not self.overload
            and attr in self
            and type(self[attr]) in [types.FunctionType, types.MethodType]
        ):
            mod = whichmodule(2)
            raise Exception(
                "lazydict - cannot change a function of method: %s - called from %s"
                % (attr, mod)
            )
        self[attr] = value
