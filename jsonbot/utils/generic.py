...
def getwho(bot, who, channel=None):
    ...
    who = who.lower()
    try:
        if bot.type in ["xmpp", "sxmpp", "sleek"]:
            return stripped(bot.userhosts[who])
        else:
            return bot.userhosts[who]
    except KeyError:
        pass
...
def decodeperchar(txt, encoding="utf-8", what=""):
    ...
    if nogo:
        if what:
            logging.debug(
                "%s: can't decode %s characters to %s" % (what, nogo, encoding)
            )
        else:
            logging.debug(
                "%s - can't decode %s characters to %s"
                % (whichmodule(), nogo, encoding)
            )
    return "".join(res)
...
def fromenc(txt, encoding="utf-8", what=""):
    ...
    try:
        ...
        logging.debug(
            "%s - can't decode %s - decoding per char" % (whichmodule(), encoding)
        )
...
def waitforqueue(queue, timeout=10000, maxitems=None, bot=None):
    ...
    try:
        while not len(queue):
            ...
            counter += 10
        logging.warn(
            "waitforqueue - result is %s items (%s) - %s"
            % (len(queue), counter, str(queue))
        )
        return queue
    except AttributeError:
        q = []
        while 1:
            ...
            counter += 10
        logging.warn(
            "waitforqueue - result is %s items (%s) - %s" % (len(q), counter, str(q))
        )
        return q
    # time.sleep(0.2)
