...
from jsonbot.lib.errors import NameNotSet, NoOwnerSet, NoUserProvided
from jsonbot.utils.name import stripname
from jsonbot.version import version
...
def makeircopts(txt=""):
    ...
    parser.add_option(
        "", "--name", type="string", default=False, dest="name", help="bot's name"
    )
    parser.add_option(
        "",
        "--port",
    ...
    parser.add_option(
        "", "--ssl", action="store_true", default=False, dest="ssl", help="use ssl"
    )
    parser.add_option(
        "-y",
        "--nossl",
    ...
    parser.add_option(
        ...
        default=False,
        dest="username",
        help="user to auth to server with",
    )
    if txt:
...
def makesxmppopts(txt=""):
    ...
    parser.add_option(
        "-u", "--user", type="string", default=False, dest="user", help="JID of the bot"
    )
    parser.add_option(
        "-o",
        "--owner",
    ...
    parser.add_option(
        ...
        default=False,
        dest="doregister",
        help="try to register the user on the server",
    )
    parser.add_option(
        "", "--name", type="string", default=False, dest="name", help="bot's name"
    )
...
def makeconsoleopts():
    ...
    parser.add_option(
        ...
        default="",
        dest="command",
        help="give a command to execute",
    )
    parser.add_option(
        "",
        "--nocolors",
    ...
    try:
        ...
        logging.warn("opts - can't parse %s" % txt)
        return
    opts.args = args
    return opts
...
def makefleetopts():
    ...
    parser = optparse.OptionParser(
        usage="usage: %prog [options] [list of bot names]", version=version
    )
    parser.add_option(
        "-a",
        "--all",
    ...
    parser.add_option(
        ...
        default=False,
        dest="show",
        help="print available fleet bots",
    )
    parser.add_option(
        "-d",
        "--datadir",
...
def makeeventopts(txt):
    ...
    parser.add_option(
        ...
        default=False,
        dest="dochan",
        help="use the channel command is given in",
    )
    parser.add_option(
        "-a",
        "--all",
        ...
        default=False,
        dest="all",
        help="use all results of the command",
    )
    parser.add_option(
        "-s",
        "--silent",
...
def makeconsoleconfig(opts=None, botname=None):
    ...
    if not botname:
        botname = opts.name or "default-console"
...
def makeircconfig(opts=None, botname=None):
    """make config file based on options."""
    if not opts:
        botname = botname or "default-irc"
    else:
        if not botname:
            botname = opts.name or "default-irc"
    botname = stripname(botname)
    from jsonbot.lib.config import Config
    ...
    if opts.nossl:
        cfg.ssl = False
    if opts.port:
        cfg.port = opts.port or cfg.port or 6667
    else:
        cfg.port = cfg.port or 6667
    if opts.server:
    ...
    if opts.username:
        ...
        cfg.username = cfg.username or "jsonbot"
    if opts.channel:
        if not opts.channel in cfg.channels:
            cfg.channels.append(opts.channel)
    else:
...
def makesxmppconfig(opts=None, botname=None, type="sxmpp"):
    """make config file based on options."""
    if not opts:
        botname = botname or "default-%s" % type
    else:
        if not botname:
            botname = opts.name or "default-%s" % type
    botname = stripname(botname)
    from jsonbot.lib.config import Config, makedefaultconfig
    ...
    if not cfg.user:
        raise NoUserProvided(
            "user is not provided .. try giving the -u option to the bot (and maybe -p as well) or run jsonbot-init and edit %s"
            % cfg.cfile
        )
    if opts.user:
        try:
    ...
    if not cfg.owner:
        raise NoOwnerSet(
            "try using the -o option or run jsonbot-init and edit %s" % cfg.cfile
        )
    if opts.nick:
        cfg.nick = opts.nick
    else:
