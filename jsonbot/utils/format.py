...
def formatevent(bot, ievent, channels, forwarded=False):
    ...
    if ievent.cmnd == "PRIVMSG":
        if ievent.txt.startswith("\001ACTION"):
            m.txt = "* %s %s" % (m.nick, ievent.txt[7:-1].strip())
        else:
            if bot.type == "irc":
                m.txt = "<%s> %s" % (m.nick, striphtml(ievent.txt))
            elif not forwarded:
                m.txt = "<%s> %s" % (m.nick, bot.normalize(ievent.txt))
            else:
                m.txt = bot.normalize(ievent.txt)
    elif ievent.cmnd == "NOTICE":
        m.target = ievent.arguments[0]
        m.txt = "-%s- %s" % (ievent.nick, ievent.txt)
    elif ievent.cmnd == "TOPIC":
        m.txt = '%s changes topic to "%s"' % (ievent.nick, ievent.txt)
    elif ievent.cmnd == "MODE":
        margs = " ".join(ievent.arguments[1:])
        m.txt = "%s sets mode: %s" % (ievent.nick, margs)
    elif ievent.cmnd == "JOIN":
        m.txt = "%s (%s) has joined %s" % (ievent.nick, ievent.userhost, ievent.channel)
    elif ievent.cmnd == "KICK":
        m.txt = "%s was kicked by %s (%s)" % (
            ievent.arguments[1],
            ievent.nick,
            ievent.txt,
        )
    elif ievent.cmnd == "PART":
        m.txt = "%s (%s) has left %s" % (ievent.nick, ievent.userhost, ievent.channel)
    elif ievent.cmnd in ("QUIT", "NICK"):
        if not ievent.user or not ievent.user.data.channels:
            logging.debug(
                "chatlog - can't find joined channels for %s" % ievent.userhost
            )
            return m
        cmd = ievent.cmnd
        ...
        if ievent.type == "unavailable":
            m.txt = "%s left" % ievent.nick
        else:
            m.txt = "%s joined" % ievent.nick
    elif ievent.cbtype == "MESSAGE":
        m.txt = "<%s> %s" % (m.nick, ievent.txt)
    elif ievent.cbtype == "OUTPUT":
        m.txt = "<%s> %s" % (bot.cfg.nick, ievent.txt)
    return m
