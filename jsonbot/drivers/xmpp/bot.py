...
from jsonbot.lib.callbacks import callbacks, remote_callbacks
from jsonbot.lib.channelbase import ChannelBase
from jsonbot.lib.errors import CannotAuth, CantLogon, NoUserProvided
from jsonbot.lib.exit import globalshutdown
from jsonbot.lib.fleet import getfleet
from jsonbot.lib.less import Less
from jsonbot.lib.threads import start_new_thread
from jsonbot.lib.users import users
from jsonbot.utils.dol import Dol
from jsonbot.utils.exception import handle_exception
from jsonbot.utils.generic import (
    fromenc,
    getrandomnick,
    jabberstrip,
    stripcolor,
...
class SXMPPBot(XMLStream, BotBase):
    ...
    def __init__(self, cfg=None, usersin=None, plugs=None, jid=None, *args, **kwargs):
        ...
        try:
            ...
            raise Exception("%s - user not set - %s" % (self.cfg.name, str(self.cfg)))
    ...
    def _keepalive(self):
        ...
        while not self.stopped:
            time.sleep(1)
            nrsec += 1
            if nrsec < (self.cfg.keepalive or 120):
                continue
            else:
                nrsec = 0
            self.sendpresence()
    ...
    def sendpresence(self):
        ...
        if self.state:
            if "status" in self.state and self.state["status"]:
                status = self.state["status"]
            else:
                status = ""
            if "show" in self.state and self.state["show"]:
    ...
    def connect(self, reconnect=False):
        ...
        if not XMLStream.doconnect(self):
            return False
        self.connection = None
        iq = self.makeready()
        if not iq:
    ...
    def register(self, jid, password):
        ...
        if not iq:
            ...
            return
        iq = self.waiter(
            """<iq type='set'><query xmlns='jabber:iq:register'><username>%s</username><resource>%s</resource><password>%s</password></query></iq>"""
            % (jid.split("@")[0], resource, password)
        )
        if iq.error:
            logging.warn("%s - register FAILED - %s" % (self.cfg.name, iq.error))
            if not iq.error.code:
                logging.error("%s - can't determine error code" % self.cfg.name)
                return False
            if iq.error.code == "405":
                logging.error(
                    "%s - this server doesn't allow registration by the bot, you need to create an account for it yourself"
                    % self.cfg.name
                )
            elif iq.error.code == "500":
                logging.error(
                    "%s - %s - %s" % (self.cfg.name, iq.error.code, iq.error.text.data)
                )
            else:
                logging.error("%s - %s" % (self.cfg.name, xmpperrors[iq.error.code]))
            self.error = iq.error
    ...
    def outnocb(
        ...
        **kwargs
        ...
        """output txt to bot."""
        if printto and printto in self.state["joinedchannels"]:
            outtype = "groupchat"
        else:
            outtype = (event and event.type) or "chat"
        target = printto
    ...
    def handle_message(self, data):
        ...
        if self.cfg.fulljids and not m.msg:
            ...
            if utarget:
                m.userhost = m.jid = m.auth = stripped(utarget)
            else:
                m.userhost = m.jid
        if m.msg:
            m.userhost = stripped(m.userhost)
    ...
    def errorHandler(self, event):
        ...
        try:
            logging.error(
                "%s - error occured in %s - %s"
                % (self.cfg.name, event.txt, event.userhost)
            )
            event.errorHandler()
        except AttributeError:
            logging.error("%s - unhandled error - %s" % (self.cfg.name, event.dump()))
    ...
    def handle_presence(self, data):
        ...
        if p.type != "unavailable":
            ...
            try:
                ...
                logging.debug(
                    "%s - removed %s channel jids" % (self.cfg.name, p.channel)
                )
            except KeyError:
                pass
        else:
    ...
    def send(self, what):
        ...
        try:
            ...
            logging.error(
                "%s - can't determine where to send %s to" % (self.cfg.name, str(what))
            )
            return
        try:
            jid = JID(to)
            ...
            logging.error(
                "%s - invalid jid - %s - %s" % (self.cfg.name, str(to), whichmodule(2))
            )
            return
        try:
            del what["from"]
        ...
        try:
            ...
            handle_exception()
            return
        if not self.checkifvalid(xml):
            logging.error("%s - NOT PROPER XML - %s" % (self.cfg.name, xml))
        else:
            self._raw(xml)

    def action(
        self, printto, txt, fromm=None, groupchat=True, event=None, *args, **kwargs
    ):
        """send an action."""
        txt = "/me " + txt
        if self.google:
    ...
    def join(self, channel, password=None, nick=None):
        ...
        q = queue.Queue()
        presence = Presence(
            {"to": channel + "/" + nick, "xmlns": "http://jabber.org/protocol/muc"}
        )
        if password:
    ...
    def topiccheck(self, msg):
        ...
        if msg.groupchat:
            try:
                ...
                logging.debug(
                    "%s - topic of %s set to %s" % (self.cfg.name, msg.channel, topic)
                )
            except AttributeError:
                return None
