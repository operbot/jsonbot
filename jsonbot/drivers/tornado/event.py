...
from jsonbot.lib.outputcache import add
from jsonbot.utils.exception import handle_exception
from jsonbot.utils.generic import fromenc, splittxt, toenc
from jsonbot.utils.lazydict import LazyDict
from jsonbot.utils.url import getpostdata
...
class TornadoEvent(EventBase):
    ...
    def parse(self, handler, request, path=None):
        ...
        if not input:
            try:
                input = request.arguments["content"] or request.arguments["cmnd"]
            except KeyError:
                pass
        self.isweb = True
        if not input:
    ...
    def parsesocket(self, handler, message):
        ...
        self.handler = handler
        try:
            data = LazyDict(json.loads(message))
        except Exception as ex:
            logging.error("failed to parse data: %s - %s" % (message, str(ex)))
            return self
