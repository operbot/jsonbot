...
from jsonbot.utils.generic import fix_format, fromenc, strippedtxt, toenc
...
class IrcEvent(EventBase):
    ...
    def parse(self, bot, rawstr):
        ...
        if not rawstr[0] == ":":
            assert bot.cfg
            splitted.insert(
                0, ":%s!%s@%s" % (bot.cfg.nick, bot.cfg.username, bot.cfg.server)
            )
            rawstr = ":%s!%s@%s %s" % (
                bot.cfg.nick,
                bot.cfg.username,
        ...
        if self.userhost:
            ...
            try:
                self.hostname = self.userhost.split("@")[1]
            except:
                self.hostname = None
        ...
        if self.channel:
            ...
            if self.channel == self.bot.cfg.nick:
                logging.warn(
                    "irc - msg detected - setting channel to %s" % self.userhost
                )
                self.msg = True
        ...
        try:
            ...
            if nr > 399 and not nr == 422:
                logging.error(
                    "%s - %s - %s - %s"
                    % (self.bot.cfg.name, self.cmnd, self.arguments, self.txt)
                )
        except ValueError:
            pass
        self.prepare()
    ...
    def reply(
        ...
        **kwargs
        ...
        if self.isdcc:
            self.bot.say(
                self.sock, txt, result, "msg", self, nr, extend, dot, *args, **kwargs
            )
        elif self.msg:
            self.bot.say(
                self.nick, txt, result, "msg", self, nr, extend, dot, *args, **kwargs
            )
        elif self.silent or (self.chan and self.chan.data and self.chan.data.silent):
            self.bot.say(
                self.nick, txt, result, "msg", self, nr, extend, dot, *args, **kwargs
            )
        else:
            self.bot.say(
                self.channel, txt, result, "msg", self, nr, extend, dot, *args, **kwargs
            )
        return self
